# Bend authority-v0 proof laboratory

**Status:** research-only FORMAL-001 adapter candidate; no runtime authority

This directory tests whether Bend 2 can mechanically check a small set of laws over the existing six-input AXIOM Rust trust-core authority abstraction.

It does **not** create a second authority engine. It is subordinate to the existing AXIOM authority/evidence boundaries and to the pending FORMAL-001 verification-substrate design in PR #1537.

## Scope

The formal model covers exactly six boolean facts:

1. principal verified;
2. capability authorized;
3. consent required;
4. consent valid;
5. effect budget required;
6. effect budget positive.
...
A passing Bend proof establishes only that these Bend laws type-check/prove under the pinned research checker source and runtime used by the workflow.

It does not establish that:

- Bend is globally sound;
- the Rust or Node implementation is proven merely because the Bend model is proven;
- the six-boolean abstraction captures all production authority semantics;
- external-world premises are true;
- any capability, mandate, approval, consent, or effect is authorized;
- FORMAL-001 is implemented or promoted;
- Bend belongs in the production trusted computing base.

Proof evidence never mints or widens authority. The supported `Gateway -> Hypervisor -> Sandbox -> Grid` path remains unchanged.
