...
- `agent-collect-benchmark-metrics-effect-admission.v1.schema.json` — short-lived Ed25519-signed admission for the existing inert `collect-benchmark-metrics:builtin` step, bound to the exact revision, lifecycle, compiler, Linux isolation policy, and fixed synthetic benchmark policy while granting no arbitrary benchmark, host-telemetry, repository-code, score, or SLO authority;
- `agent-collect-benchmark-metrics-effect-receipt.v1.schema.json` — signed executor-originated evidence for one exact fixed synthetic workload, its deterministic checksum, one bounded monotonic elapsed observation, consume/final durable heads, Linux isolation image, and cleanup while keeping machine-comparison, production-SLO, task-success, general-executor, deployment, capability, and AXIOM-authority claims false.
...
The **plan-bound `collect-benchmark-metrics` laboratory** adds a third operation-specific gate without creating a generic benchmark runner. It reuses the compiler's inert `collect-benchmark-metrics:builtin` step; the parent authorization and dry-run plan remain `effect_reachable:false`. A separate short-lived Ed25519 admission binds the exact plan/revision/lifecycle/compiler/Linux isolation policy and fixed `synthetic-lcg-u32-262144-v1` benchmark policy. The controller again requires exact signed `issued` lifecycle evidence and known-active laboratory revocation input, commits durable generation 2 `consumed`, and verifies a separately signed consumed durable-head receipt before exposing the fixed effect descriptor.

The adapter runs only one content-digested Node script in the reviewed disposable Linux container. The workload is `lcg-u32-262144-v1`: exactly 262,144 deterministic integer iterations starting from the fixed seed encoded in the script and requiring checksum `1679840888`. The only nondeterministic metric is elapsed nanoseconds from `process.hrtime.bigint()`. The adapter reads no host CPU model, CPU counters, memory totals, process inventory, kernel/hardware telemetry, `/proc`, `/sys`, repository files, or caller data. It accepts no caller benchmark code, input, command, argv, shell, environment-controlled executable, bind mount, network origin, credential/secret source, package/service operation, remote endpoint, or contributed hardware.

The metrics projection contains exactly six fields: `benchmark_policy_id`, `workload_id`, `iterations`, `checksum_u32`, `timer_source`, and `elapsed_ns`. The elapsed value is evidence about that one isolated hosted run only. No throughput, normalized score, machine ranking, production SLO, capacity guarantee, or application success is derived from it. A valid signed executor receipt may claim only that the fixed synthetic process ran, the deterministic checksum matched, the bounded monotonic elapsed observation was recorded, durable consume-before-effect ordering held, and cleanup was verified. Machine-comparison score, host telemetry, production SLO, repository code, network, credentials/secrets, general executor, global currentness, deployment, capability-promotion, task-success, and AXIOM-authority claims remain false.
