...
const projectionUrl = new URL('../../institutional/domain-projections.v1.json', import.meta.url);
...
test('pattern applicability domains reference canonical domain projection keys', async () => {
  const projections = JSON.parse(await readFile(projectionUrl, 'utf8'));
  const { pattern } = await load();
  const domainIds = new Set(Object.keys(projections.domains));

  for (const domainId of pattern.applicability.domains) {
    assert.ok(domainIds.has(domainId), `unknown institutional domain projection ${domainId}`);
  }
});
