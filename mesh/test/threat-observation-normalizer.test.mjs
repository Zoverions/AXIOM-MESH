...
test('withdrawn and fixed-upstream evidence remains present in corpus history', () => {
  for (const state of ['source_withdrawn', 'fixed_upstream']) {
    const observation = normalizedObservation({
      observation_id: `obs:${state}:1`,
      lifecycle_state: state
    });
    const [entry] = mergeOfflineObservationCorpus([observation], [], { now: NOW });
    assert.equal(entry.observation.observation_digest, observation.observation_digest);
    assert.equal(entry.derived_lifecycle_state, state);
  }
});

test('lifecycle projection never creates authority-bearing fields', () => {
  const observation = normalizedObservation({ observation_id: 'obs:no-authority:1' });
  const [entry] = mergeOfflineObservationCorpus([observation], [], { now: NOW });
  for (const forbidden of ['capability', 'authorize', 'execute', 'policy_patch', 'containment_instruction']) {
    assert.equal(Object.hasOwn(entry, forbidden), false);
    assert.equal(Object.hasOwn(entry.observation, forbidden), false);
  }
});
