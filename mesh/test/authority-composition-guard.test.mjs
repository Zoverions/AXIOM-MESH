...
test('ordered composition restrictions preserve repetition and non-set semantics', () => {
  const result = evaluateAuthorityComposition({
    grant: grant(),
    intent: intent(),
    ...
    restrictions: [{
      id: 'read-summary-read',
      ordered_actions: ['records.read', 'records.summarize', 'records.read']
    }],
    now: NOW
  });

  assert.equal(result.allow, false);
  assert.ok(result.reasons.includes('composition-blocked:read-summary-read'));
});
