...
function historyEntry(action, destination = 'destination:local') {
  return {
    causal_scope_id: 'causal:user-request-42',
    action,
    resource: 'resource:records',
...
test('intent can narrow a verified grant but cannot widen it', () => {
  ...
  const widened = verifyIntentAttenuation(grant(), intent({ actions: ['records.read', 'records.summarize', 'shell.exec'] }));
...
test('verified identity or protocol context cannot repair a missing verified grant', () => {
  assert.throws(() => evaluateAuthorityComposition({ grant: grant({ verified: false }), intent: intent(), request: request(), now: NOW }), /must be independently verified/);
});
...
test('resource, policy, principal and intent mismatches fail closed', () => {
  for (const candidate of [
    ...
    const result = evaluateAuthorityComposition({ grant: grant(), intent: intent(), request: candidate, now: NOW });
...
test('individually permitted actions are denied when causal composition is prohibited', () => {
  const result = evaluateAuthorityComposition({
    grant: grant(),
    intent: intent({ actions: ['records.read', 'records.summarize', 'send.external'], destinations: ['destination:external', 'destination:local'] }),
    request: request({ action: 'send.external', destination: 'destination:external', protocol: 'a2a' }),
    history: [historyEntry('records.read')],
    restrictions: [{ id: 'private-read-then-external-send', ordered_actions: ['records.read', 'send.external'] }],
    now: NOW
  });
  assert.equal(result.allow, false);
...
test('ordered restrictions preserve non-lexicographic order and repeated actions', () => {
  const result = evaluateAuthorityComposition({
    grant: grant(),
    intent: intent({ actions: ['records.read', 'records.summarize', 'send.external'], destinations: ['destination:external', 'destination:local'] }),
    request: request({ action: 'records.read' }),
    history: [historyEntry('records.read'), historyEntry('send.external', 'destination:external')],
    restrictions: [{ id: 'read-send-read', ordered_actions: ['records.read', 'send.external', 'records.read'] }],
    now: NOW
  });
  assert.equal(result.allow, false);
...
test('transport or protocol session splitting does not erase causal authority history', () => {
  const result = evaluateAuthorityComposition({
    grant: grant(),
    intent: intent({ actions: ['records.read', 'records.summarize', 'send.external'], destinations: ['destination:external', 'destination:local'] }),
    request: request({ action: 'send.external', destination: 'destination:external', protocol: 'native-gateway' }),
    history: [historyEntry('records.read')],
    restrictions: [{ id: 'private-read-then-external-send', ordered_actions: ['records.read', 'send.external'] }],
    now: NOW
  });
...
test('unrelated causal work does not contaminate this authority scope', () => {
  const result = evaluateAuthorityComposition({
    ...
    history: [{ ...historyEntry('records.read'), causal_scope_id: 'causal:other-task' }],
    restrictions: [{ id: 'read-then-summary-block-demo', ordered_actions: ['records.read', 'records.summarize'] }],
    now: NOW
  });
