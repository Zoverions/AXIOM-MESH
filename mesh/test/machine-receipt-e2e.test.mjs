...
function machinePrincipal(id, sponsor, runtimeId) {
  return {
    ...
    runtime: { id: runtimeId, kind: 'local-process', software_digest: 'd'.repeat(64) },
    constraints: {
      actions: ['system.echo'], purposes: ['test.conformance'], destinations: ['local'],
      budgets: {
        max_requests_per_minute: 60, max_concurrent_requests: 2,
        max_execution_ms: 2_000, max_request_bytes: 65_536, max_response_bytes: 262_144
      },
...
function apiTokens() {
  return {
    [HUMAN_TOKEN]: { id: 'owner.receipt-e2e', type: 'human', roles: ['administrator'], scopes: ['*'] },
    [AGENT_TOKEN]: machinePrincipal('agent.receipt-e2e', 'owner.receipt-e2e', 'runtime.receipt-e2e'),
    [OTHER_AGENT_TOKEN]: machinePrincipal('agent.receipt-other', 'owner.receipt-e2e', 'runtime.receipt-other')
  };
}
...
async function stackFixture(t) {
  ...
  t.after(async () => {
    ...
    finally { await lease.release(); await rm(dataDir, { recursive: true, force: true }); }
  });
  stack = await startDevelopmentStack({
    dataDir, environment: 'test', autoBootstrap: true,
    gatewayPort: basePort, hypervisorPort: basePort + 1, sandboxPort: basePort + 2, gridPort: basePort + 3,
    hypervisorUrl: `http://127.0.0.1:${basePort + 1}`,
    sandboxUrl: `http://127.0.0.1:${basePort + 2}`,
    gridUrl: `http://127.0.0.1:${basePort + 3}`,
    rateLimitCapacity: 1_000, rateLimitRefillPerSecond: 1_000, apiTokens: apiTokens()
  });
  const gateway = `http://127.0.0.1:${basePort}`;
  const client = token => createGatewayClient({ token, request: (path, options) => fetch(`${gateway}${path}`, options) });
  return { dataDir, machine: client(AGENT_TOKEN), otherMachine: client(OTHER_AGENT_TOKEN), human: client(HUMAN_TOKEN) };
}
...
test('real Gateway returns an owner-scoped Grid-attested machine receipt that verifies independently', async t => {
  ...
  const result = await machine.call('intents.submit', {
    body: { action: 'system.echo', input: { message: 'receipt-bound-output' }, purpose: 'test.conformance' },
    idempotencyKey: 'machine-receipt-e2e-0001'
  });
  assert.equal(result.status, 'completed');

  const receipt = await machine.call('machine_receipts.verify', { params: { id: result.intent_id } });
  ...
  assert.deepEqual(receipt.statement.evidence_events.map(event => event.kind), ['intent.accepted', 'intent.completed']);
  ...
  await assert.rejects(
    () => otherMachine.call('machine_receipts.verify', { params: { id: result.intent_id } }),
    error => error.code === 'not_found' && error.status === 404
  );
  await assert.rejects(
    () => otherMachine.call('machine_receipts.verify', { params: { id: `intent_${'f'.repeat(64)}` } }),
    error => error.code === 'not_found' && error.status === 404
  );
  await assert.rejects(
    () => human.call('machine_receipts.verify', { params: { id: result.intent_id } }),
