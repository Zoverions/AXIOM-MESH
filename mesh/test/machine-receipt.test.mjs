...
test('machine receipt binds intent, terminal evidence, chain assurance, and Grid attestation', () => {
  ...
  const { intent, events, chain, result } = fixture();
  const receipt = buildMachineIntentReceipt({
    intent,
    events,
    chain,
    identity,
  ...
  assert.equal(receipt.statement.chain.verification_mode, 'checkpoint');
  assert.equal(receipt.statement.chain.prefix_assurance, 'signed_checkpoint');
  assert.equal(receipt.statement.chain.full_verification_required_for_checkpointed_prefix_revalidation, true);
  assert.deepEqual(receipt.statement.verification, {
    owner_bound: true,
    request_bound: true,
    terminal_bound: true,
  ...
  assert.equal(verified.intent_id, intent.intent_id);
...
test('machine receipt verifies denied terminal evidence without exposing raw error', () => {
  const identity = gridIdentity();
  const { intent, events, chain, error } = fixture({ status: 'denied' });
  const receipt = buildMachineIntentReceipt({
    intent,
    events,
    chain,
  ...
  assert.equal(receipt.statement.outcome.kind, 'intent.denied');
...
test('machine receipt fails closed on incomplete, mismatched, or nonterminal evidence', () => {
  const identity = gridIdentity();
  const { intent, events, chain } = fixture();

  assert.throws(
    () => buildMachineIntentReceipt({
      intent: { ...intent, status: 'accepted' },
      events,
      chain,
      identity,
      kernelVersion: '0.12.0-dev.3'
    }),
    error => error.code === 'receipt_not_ready' && error.status === 409
  );
  ...
  assert.throws(
    ...
    error => error.code === 'receipt_evidence_incomplete' && error.status === 409
  );

  const altered = structuredClone(events);
  altered[1].payload.result.message = 'substituted';
  assert.throws(
    () => buildMachineIntentReceipt({
      intent,
      events: altered,
    ...
    error => error.code === 'receipt_evidence_mismatch' && error.status === 409
  );
  ...
  assert.throws(
    () => buildMachineIntentReceipt({
      intent,
      events,
      chain: { valid: false, reason: 'signature_mismatch' },
      identity,
      kernelVersion: '0.12.0-dev.3'
    }),
...
test('machine receipt digest and Grid signature detect substitution', () => {
  ...
  tamperedDigest.statement.intent.action = 'system.hash';
  assert.throws(
    () => validateMachineIntentReceipt(tamperedDigest),
    /digest does not match/
  );

  const tamperedSignature = structuredClone(receipt);
  tamperedSignature.statement.intent.action = 'system.hash';
  const envelope = {
    schema: tamperedSignature.schema,
    statement: tamperedSignature.statement,
    attestation: tamperedSignature.attestation
  };
