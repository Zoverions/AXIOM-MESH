...
test('one command_id cannot be rebound to a different source authorization', async () => {
  const dir = await mkdtemp(join(tmpdir(), 'axiom-provisioning-command-id-'));
  const witness = keys();
  const operator = keys();
  ...
  const admission2 = createPublicWitnessSourceAdmission({
    domainId: 'axiom.social.public.v1',
    sourceId: 'command-id-source-two',
  ...
  const first = createPublicWitnessSourceProvisioningCommand({
    ...
    operatorId: 'command-id-operator',
  ...
  const facade = {
    ...
    admitSource: async (value, options) => {
      if (fail) throw new Error('hold first authorization');
  ...
  const store = await openPublicWitnessSourceProvisioningApplicationStore({
    ...
    provisionerId: 'command-id-provisioner',
    provisionerPrivateKey: provisioner.privateKey
  });
  await assert.rejects(store.apply(first, { appliedAt: '2026-08-17T23:01:00.000Z' }), /hold first authorization/);
  fail = false;
  await assert.rejects(
    store.apply(second, { appliedAt: '2026-08-17T23:02:00.000Z' }),
    /command_id is already bound/
  );
