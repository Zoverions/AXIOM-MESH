...
test('Response Assurance Envelope v0 schema is strict evidence-only and non-authorizing', async () => {
  ...
  assert.ok(schema.required.includes('claim_adjudication_digest'));
  assert.ok(schema['x-axiom-semantic-rules'].some(rule => /verifier_digest/i.test(rule)));
  assert.ok(schema['x-axiom-semantic-rules'].some(rule => /claim_adjudication_digest|claim adjudication/i.test(rule)));
