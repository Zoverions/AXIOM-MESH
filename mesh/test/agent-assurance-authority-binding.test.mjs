...
test('request identity binds assurance digest so deny and non-deny evidence cannot collide', () => {
  const machine = principal();
  const safeEvidence = evidence(machine);
  const denyEvidence = evidence(machine, {
    environment: {
      declared: evidence(machine).environment.declared,
      observed: {
        ...evidence(machine).environment.observed,
  ...
  const safeIntent = {
    principal: machine,
    action: 'system.echo',
    input: { message: 'identity' },
    purpose: 'test.conformance',
    data_scopes: [],
    assurance_evidence: safeEvidence
  };
  const denyIntent = {
    ...safeIntent,
    assurance_evidence: denyEvidence
  };

  const safeSignal = evaluateAgentAssuranceDenySignal(safeEvidence, machine);
  const denySignal = evaluateAgentAssuranceDenySignal(denyEvidence, machine);
  assert.equal(safeSignal.deny, false);
  assert.equal(denySignal.deny, true);

  assert.equal(
    intentRequestBinding(safeIntent).agent_assurance_digest,
    digestObject(normalizeAgentAssuranceEvidence(safeEvidence))
  );
  assert.notEqual(
    intentRequestDigest(safeIntent),
    intentRequestDigest(denyIntent)
  );

  // Assurance still cannot turn a machine deny into allow.
  const machineDenied = evaluateMachineIntentWithAssurance(machine, machineIntentOptions({
    action: 'system.delete',
    assurance_evidence: safeEvidence
  }));
  assert.equal(machineDenied.allow, false);
  assert.equal(machineDenied.code, 'machine_action_denied');
  assert.equal(machineDenied.assurance, undefined);
});
