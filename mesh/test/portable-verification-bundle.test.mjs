...
test('portable verification bundle requires at least one trust anchor', async () => {
  const bundle = await exampleBundle();
  bundle.trust_material.trust_anchor_digests = [];
  assert.throws(
    () => validatePortableVerificationBundle(bundle, {
      now: new Date('2026-09-01T13:00:00.000Z')
    }),
    /trust_material requires trust_anchor_digests/
  );
});

test('bundle age policy rejects a one-millisecond overflow without rounding grace', async () => {
  const bundle = await exampleBundle();
  bundle.freshness_requirements.max_bundle_age_seconds = 3600;
  const result = validatePortableVerificationBundle(bundle, {
    now: new Date('2026-09-01T13:20:00.001Z')
  });
  assert.equal(result.checks.bundle_age_within_policy, false);
  assert.equal(result.valid, false);
});

test('future created_at cannot satisfy bundle age policy', async () => {
  const bundle = await exampleBundle();
  bundle.created_at = '2026-09-01T13:00:00.000Z';
  const result = validatePortableVerificationBundle(bundle, {
