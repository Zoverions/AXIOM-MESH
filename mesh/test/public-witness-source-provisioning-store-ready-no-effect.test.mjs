...
test('effect-ready is durable intent to attempt, not evidence that W2c2 admitted the source', async () => {
  const dir = await mkdtemp(join(tmpdir(), 'axiom-provisioning-ready-no-effect-'));
  const witness = keys();
  const operator = keys();
  ...
  const command = createPublicWitnessSourceProvisioningCommand({
    ...
    operatorId: 'ready-no-effect-operator',
    operatorPrivateKey: operator.privateKey,
    commandId: 'ready-no-effect-command',
  ...
  const facade = {
    ...
    admitSource: async () => { throw new Error('no receiver effect'); }
  };
  const store = await openPublicWitnessSourceProvisioningApplicationStore({
    ...
    provisionerId: 'ready-no-effect-provisioner',
  ...
  assert.equal(status.authorization_retained, true);
  assert.equal(status.ready_attempt_count, 1);
  assert.equal(status.linked, false);
  assert.equal(status.receiver_admission_retained, false);
