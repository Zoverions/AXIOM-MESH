...
test('identity lineage fixtures keep discovery, continuity and accountability non-authoritative', async () => {
  ...
  assert.equal(threat.schema, 'axiom-identity-lineage-composition-threat-model.v1');
  assert.ok(Array.isArray(threat.planes), 'threat model planes must be an array');
  assert.deepEqual(
    threat.planes.map(({ id }) => id),
    ['endpoint_currency', 'identity_continuity', 'accountability', 'authorization']
  );
  assert.deepEqual(
    threat.planes.map(({ authority_effect }) => authority_effect),
    ['none', 'none', 'none', 'bounded_local_grant_only']
  );
  assert.equal(
    threat.planes.filter(({ authority_effect }) => authority_effect !== 'none').length,
    1,
    'authorization must be the only authority-bearing plane'
  );
...
test('service-key rotation preserves identity continuity without inheriting retired-key grants', async () => {
  ...
  assert.ok(rotation, 'service-key rotation fixture must exist');
  assert.ok(oldGrant, 'retired-key grant fixture must exist');
...
test('root compromise and platform credential replay are treated as wider-scope failures', async () => {
  ...
  assert.ok(root, 'root-compromise fixture must exist');
  assert.ok(platform, 'platform credential isolation fixture must exist');
