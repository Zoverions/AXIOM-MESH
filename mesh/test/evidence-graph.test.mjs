...
test('evidence context preserves support, contradiction, alternatives, missing evidence, and challenge around a hypothesis', () => {
  const assertions=[node('assertion:h','hypothesis','Hypothesis A'),node('assertion:s','evidence-item','Supports A'),node('assertion:c','counterevidence','Conflicts with A'),node('assertion:a','alternative-explanation','Alternative B'),node('assertion:m','missing-evidence','Expected source is not yet available'),node('assertion:q','challenge','A is disputed')];
  const links=[link('link:s','assertion:s','assertion:h','supports'),link('link:c','assertion:c','assertion:h','contradicts'),link('link:a','assertion:a','assertion:h','alternative-to'),link('link:m','assertion:m','assertion:h','missing-from'),link('link:q','assertion:q','assertion:h','challenged-by')];
  ...
  assert.deepEqual(new Set(result.links.map(item=>item.relation)),new Set(['supports','contradicts','alternative-to','missing-from','challenged-by']));
});

test('review state keeps availability, machine review, human review, reliance, disclosure, and adjudication independent', () => {
  const review=validateEvidenceReviewState({ schema:EVIDENCE_REVIEW_SCHEMA, object_ref:'artifact:1', known:true, available:true, acquired:true, integrity_verified:true, indexed:true, machine_reviewed:true, human_reviewed:false, relied_upon:false, disclosed:true, challenged:false, adjudicated:false, updated_at:at });
  ...
  assert.equal(review.adjudicated,false);
});
