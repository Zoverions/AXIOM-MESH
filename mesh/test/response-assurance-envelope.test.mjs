...
function envelope(profileDocument, overrides = {}) {
  const item = {
    ...
    claim_adjudication_digest: null,
...
test('dedupes supporting verifier digests before counting independence groups', () => {
  const p = profile();
  const item = envelope(p);
  item.verifiers = [{
    verifier_id: 'verifier.alpha.v1',
    ...
    verifier_id: 'verifier.alpha-alias.v1',
    verifier_digest: E,
    source_class: 'semantic-verifier',
    independence_group: 'semantic-lineage-b',
    result: 'supports'
  }];
  item.claimed_independent_confirmations = 2;
  assert.throws(
    () => computeResponseAssuranceEnvelopeDigest(item),
    /independence group|verifier_digest|digest/i
  );

  item.verifiers[1].independence_group = 'semantic-lineage-a';
  item.claimed_independent_confirmations = 1;
  item.envelope_digest = computeResponseAssuranceEnvelopeDigest(item);
  const resolved = resolveResponseAssuranceEnvelope(item, p, { now: '2026-09-16T23:00:00.000Z' });
  assert.equal(resolved.independent_confirmation_count, 1);
});
