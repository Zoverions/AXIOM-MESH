...
test('independently approved execution renders A3 and binds approval evidence', () => {
  const approval = {
...
test('plan tampering cannot substitute assurance without invalidating validation', () => {
  const plan = buildPlan(intent(), decision());
  const tampered = structuredClone(plan);
  tampered.assurance.achieved = 'A3';
  assert.throws(() => validatePlan(tampered), /independent approval|capability declaration/i);
});

test('invalid assurance cannot enter the invocation envelope', () => {
  assert.throws(
    () => buildNativeInvocationEnvelope(intent(), decision({ required_assurance: 'A9' })),
    /required assurance must be one of/i
  );
});
