...
const profileUrl = new URL('../../agent-commons/obligation-contract-profile.v1.json', import.meta.url);
...
test('obligation deadline cannot extend beyond contract expiry', async () => {
  const contract = JSON.parse(await readFile(exampleUrl, 'utf8'));
  contract.obligations[0].deadline = '2026-09-09T13:20:00.000Z';
  assert.throws(
    () => validateObligationContract(contract),
    /deadline cannot exceed contract expires_at/
  );
});
...
test('obligation profile fixes the authority layers and settlement state vocabulary', async () => {
  const profile = JSON.parse(await readFile(profileUrl, 'utf8'));
  assert.equal(profile.schema, 'axiom-obligation-contract-profile.v1');
  assert.deepEqual(profile.settlement_states, [
    'draft','offered','accepted_as_record','active_obligation_set','partially_fulfilled',
    'fulfilled','claimed_breach','in_dispute','cure_pending','settled','terminated','expired'
  ]);
  assert.deepEqual(
    profile.contract_layers.map(({ id, authority_effect }) => ({ id, authority_effect })),
    [
      { id: 'agreement_expression', authority_effect: 'none' },
      { id: 'legal_or_institutional_context', authority_effect: 'none' },
      { id: 'obligation_state', authority_effect: 'none' },
      { id: 'execution_request', authority_effect: 'none' },
      { id: 'effect_admission', authority_effect: 'bounded_local_grant_only' }
    ]
  );
});
