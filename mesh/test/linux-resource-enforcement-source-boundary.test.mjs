...
const DRILL = new URL('../src/linux-resource-enforcement-drill.mjs', import.meta.url);
const RUNNER = new URL('../src/run-linux-resource-enforcement-drill.mjs', import.meta.url);
...
test('real G3 effect code uses fixed Linux binaries, no shell, and no caller command surface', async () => {
  ...
  assert.match(drill, /\[\.\.\.enforcement\.argv_prefix, SLEEP, '30'\]/);
  assert.doesNotMatch(drill, /shell:\s*true/);
  assert.doesNotMatch(drill, /\bexec\s*\(/);
  assert.doesNotMatch(drill, /\bspawn\s*\(/);
  assert.match(runner, /AXIOM_HOST_RESOURCE_ENFORCEMENT_LAB\s*===\s*'1'/);
  assert.doesNotMatch(runner, /process\.argv\.slice\(4/);
  assert.doesNotMatch(runner, /command|executable|argv/i);
});
