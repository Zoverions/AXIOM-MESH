...
const REQUIRED_KERNEL_SCRIPTS = Object.freeze({
  ...
  check:
    'node src/setup.mjs check && node src/check-service-network-policy.mjs && node src/check-registry.mjs && node src/status.mjs && node src/check-docs.mjs && node --test --test-reporter=spec',
