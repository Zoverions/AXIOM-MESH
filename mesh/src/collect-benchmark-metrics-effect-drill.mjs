...
} from './lib/agent-collect-benchmark-metrics-effect-admission.mjs';
import {
  ...
  AGENT_COLLECT_BENCHMARK_METRICS_MAX_OUTPUT_BYTES,
  AGENT_COLLECT_BENCHMARK_METRICS_POLICY_ID,
  AGENT_COLLECT_BENCHMARK_METRICS_WORKLOAD_ID,
  AgentCollectBenchmarkMetricsEffectController,
  verifyAgentCollectBenchmarkMetricsEffectReceipt
} from './lib/agent-collect-benchmark-metrics-effect.mjs';

const REVISION = process.env.GITHUB_SHA;
const LAB_OPT_IN = process.env.AXIOM_AGENT_COLLECT_BENCHMARK_METRICS_EFFECT_LAB;
...
const NONCE_PREFIX = 'collect_benchmark_metrics_effect_nonce_';
...
function docker(args, { timeout = PROCESS_TIMEOUT_MS, maxBuffer = AGENT_COLLECT_BENCHMARK_METRICS_MAX_OUTPUT_BYTES } = {}) {
...
function executeBenchmark(imageId, descriptor) {
  assert(descriptor.absolute_executable === AGENT_LINUX_ISOLATION_ENTRYPOINT, 'benchmark descriptor executable drifted');
  assert(descriptor.adapter_script_sha256 === AGENT_COLLECT_BENCHMARK_METRICS_ADAPTER_SCRIPT_SHA256, 'benchmark descriptor script digest drifted');
  assert(descriptor.workload_id === AGENT_COLLECT_BENCHMARK_METRICS_WORKLOAD_ID, 'benchmark descriptor workload drifted');
  assert(descriptor.iterations === AGENT_COLLECT_BENCHMARK_METRICS_ITERATIONS, 'benchmark descriptor iterations drifted');
  const name = `axiom-benchmark-${randomBytes(6).toString('hex')}`;
  let result;
  let cleanupVerified = false;
  try { result = docker(fixedArgs(name, imageId)); }
  finally { cleanupVerified = cleanup(name); }
  assert(cleanupVerified, 'benchmark effect container cleanup could not be verified');
  if (result.error || result.status !== 0) fail(`benchmark effect failed: ${String(result.stderr || result.error?.message || '').slice(0, 1200)}`);
  const stderr = String(result.stderr || '');
  assert(stderr.length === 0, 'benchmark effect wrote stderr');
  const output = String(result.stdout || '').trim();
  assert(output.length > 0 && Buffer.byteLength(output, 'utf8') <= AGENT_COLLECT_BENCHMARK_METRICS_MAX_OUTPUT_BYTES, 'benchmark effect output is invalid');
  let parsed;
  try { parsed = JSON.parse(output); } catch { fail('benchmark effect output is not JSON'); }
  assert(parsed?.benchmark_policy_id === AGENT_COLLECT_BENCHMARK_METRICS_POLICY_ID, 'benchmark policy drifted');
  assert(parsed?.workload_id === AGENT_COLLECT_BENCHMARK_METRICS_WORKLOAD_ID, 'benchmark workload drifted');
  assert(parsed?.iterations === AGENT_COLLECT_BENCHMARK_METRICS_ITERATIONS, 'benchmark iteration count drifted');
  assert(parsed?.checksum_u32 === AGENT_COLLECT_BENCHMARK_METRICS_EXPECTED_CHECKSUM, 'benchmark checksum drifted');
  assert(parsed?.timer_source === 'process.hrtime.bigint', 'benchmark timer source drifted');
  assert(Number.isSafeInteger(parsed?.elapsed_ns) && parsed.elapsed_ns > 0, 'benchmark elapsed metric is invalid');
  return Object.freeze({
    adapter_script_sha256: AGENT_COLLECT_BENCHMARK_METRICS_ADAPTER_SCRIPT_SHA256,
    benchmark_policy_id: parsed.benchmark_policy_id,
    workload_id: parsed.workload_id,
    iterations: parsed.iterations,
    checksum_u32: parsed.checksum_u32,
    timer_source: parsed.timer_source,
...
function objects(nowMs) {
  const offerId = 'offer:collect-benchmark-metrics:hosted-ci';
  const challengeId = 'infra:collect-benchmark-metrics:effect';
  ...
  const offer = {
    ...
    publisher: { type: 'human', id: 'contributor:hosted-ci-benchmark-lab' },
    node_profile: { schema: 'axiom-compute-node-profile.v1', profile_id: 'node:hosted-ci:benchmark', profile_sha256: PROFILE_SHA },
  ...
  const challenge = {
    ...
    evidence_requirements: ['Bind exact plan, consumed durable head, fixed benchmark script digest and final effect receipt.'],
  ...
  const attestationStatement = {
    attestation_id: 'attestation:collect-benchmark-metrics:hosted-ci', repository: 'Zoverions/AXIOM-MESH', offer_id: offerId,
  ...
  const attestation = {
    ...
    evidence_refs: ['evidence:key-possession:hosted-ci-benchmark-lab'],
  ...
  const authorization = {
    schema: 'axiom-agent-test-session-authorization.v1', authorization_id: 'session-auth:collect-benchmark-metrics:hosted-ci', repository: 'Zoverions/AXIOM-MESH',
    sponsor: { type: 'human', id: 'sponsor:agent-commons-benchmark-lab', approval_ref: 'approval:issue:1155' }, subject: { type: 'machine', id: 'agent:hosted-ci-benchmark' },
    ...
    revocation: { revocable: true, one_time: true, fail_closed_on_unknown: true, revocation_ref: 'revocation:collect-benchmark-metrics:hosted-ci' },
...
function preflight() {
  assert(process.platform === 'linux', 'benchmark effect laboratory requires Linux');
  assert(LAB_OPT_IN === '1', 'benchmark effect laboratory requires explicit opt-in');
  assert(process.env.CI === 'true' && process.env.GITHUB_ACTIONS === 'true', 'benchmark effect laboratory is hosted-CI-only');
...
function main() {
  ...
  const ledger = new AgentTestSessionLifecycleLedger({ ledgerId: 'session-ledger:collect-benchmark-metrics:hosted-ci', ledgerPrivateKey: lifecycleKeys.privateKey });
  ledger.issue(authorization, { eventId: 'event:collect-benchmark-metrics:issued', occurredAt: authorization.timing.issued_at, challenge, offer, attestation, expectedNonce: nonce, now: new Date(now) });
  const lifecycleTranscript = ledger.exportTranscript();
  const lifecycleReceipt = ledger.receipt({ generatedAt: iso(nowMs - 2_000) });
  const platformProfile = {
    schema: 'axiom-agent-executor-platform-profile.v1', profile_id: `platform:hosted-ci-benchmark:linux-${arch()}`,
  ...
  assert(plan.effects.effect_reachable === false && plan.steps.length === 1 && plan.steps[0].step_id === 'collect-benchmark-metrics:builtin', 'benchmark dry-run plan boundary changed');

  const issuerKeys = keyPair();
  const admission = createAgentCollectBenchmarkMetricsEffectAdmission({
    admissionId: 'effect-admission:collect-benchmark-metrics:hosted-ci', issuerId: 'issuer:agent-commons-hosted-ci-benchmark-lab', issuerPrivateKey: issuerKeys.privateKey,
    plan, revision: REVISION, notBefore: iso(nowMs - 1_000), expiresAt: iso(nowMs + 120_000)
  });
  verifyAgentCollectBenchmarkMetricsEffectAdmission(admission, { trustedIssuerPublicKey: issuerKeys.publicKey, plan, expectedRevision: REVISION, now });

  const stateRoot = mkdtempSync(join(resolve(RUNNER_TEMP), 'axiom-collect-benchmark-metrics-state-'));
  ...
  try {
    store = AgentExecutorDurableStateStore.open({
      stateRoot, storeId: 'executor-durable:collect-benchmark-metrics:hosted-ci', storePrivateKey: storeKeys.privateKey,
    ...
    controller = new AgentCollectBenchmarkMetricsEffectController({
    ...
    consumed = true;
    assert(store.status === 'consumed' && store.generation === 2, 'benchmark durable consume was not committed before effect');
    ...
    assert(checkedConsumedHead.statement.lifecycle_status === 'consumed' && checkedConsumedHead.statement.generation === 2, 'benchmark signed consumed head is invalid before effect');
    const effectObservation = executeBenchmark(isolationReceipt.adapter.image_id, descriptor);
    const effectReceipt = controller.complete({ observation: effectObservation, finishedAt: iso(Date.now()) });
    verifyAgentCollectBenchmarkMetricsEffectReceipt(effectReceipt, {
    ...
    assert(checkedDurableHead.statement.generation === effectReceipt.statement.durable_final_generation, 'benchmark final durable head generation mismatch');
    assert(checkedDurableHead.statement.record_digest === effectReceipt.statement.durable_final_record_digest, 'benchmark final durable head digest mismatch');
    const bundleBody = {
      schema: 'axiom-agent-collect-benchmark-metrics-effect-evidence-bundle.v1', revision: REVISION, plan, admission,
      ...
      claims: {
        host_telemetry_read: false, machine_comparison_score_claimed: false, production_slo_claimed: false,
        arbitrary_benchmark_used: false, repository_code_executed: false,
    ...
    if (consumed && controller && store?.status === 'consumed') {
      try { controller.interrupt({ occurredAt: iso(Date.now()), reasonCode: 'collect-benchmark-metrics-effect-drill-failed' }); } catch {}
    }
    throw error;
  } finally {
...
try { main(); }
  ...
  process.stderr.write(`Collect-benchmark-metrics effect drill failed: ${String(error?.message || error).slice(0, 2000)}\n`);
  process.exit(1);
}
