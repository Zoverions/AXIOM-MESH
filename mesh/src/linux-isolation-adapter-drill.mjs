...
const BASELINE_PROBE = String.raw`
...
function statusValue(name) {
  ...
  if (!match) throw new Error('missing proc status field ' + name);
  return match[1].trim();
}
function mountOption(mountPoint) {
  const line = fs.readFileSync('/proc/mounts', 'utf8').split('\n').find(item => {
    const fields = item.split(' ');
    return fields[1] === mountPoint;
  });
  if (!line) throw new Error('missing mount ' + mountPoint);
...
(async () => {
  ...
  const fdTargets = fs.readdirSync('/proc/self/fd').map(name => {
    try { return fs.readlinkSync('/proc/self/fd/' + name); } catch { return ''; }
  });
  const cpuFields = fs.readFileSync('/sys/fs/cgroup/cpu.max', 'utf8').trim().split(/\\s+/);
  const observation = {
    ...
    root_read_only: mountOption('/').includes('ro'),
    ...
    unexpected_sensitive_fd: fdTargets.some(target =>
      target.includes('docker.sock') || target.includes('/run/secrets') || target.includes('/home/runner') || target.includes('/_work/')
    ),
...
const PID_CEILING_PROBE = String.raw`
...
const TIMEOUT_PROBE = String.raw`
const { spawn } = require('node:child_process');
spawn('/bin/sleep', ['30'], { stdio: 'ignore' });
setInterval(() => {}, 1000);
`;

const OUTPUT_PROBE = String.raw`
...
function runJsonProbe(probeId, script) {
  ...
  try {
    const result = runDocker(fixedContainerArgs(name, script));
    return parseJsonOutput(result, probeId);
  } finally {
...
function validateBaseline(observation, runnerNamespaces) {
  ...
  assert(Number.isInteger(observation.cpu_quota) && Number.isInteger(observation.cpu_period), 'CPU cgroup evidence was invalid');
...
function runPidCeilingProbe() {
  ...
  try {
    const result = runDocker(fixedContainerArgs(name, PID_CEILING_PROBE));
    observation = parseJsonOutput(result, 'pid-ceiling');
  } finally {
...
function preflight() {
  ...
  const mode = statSync(AGENT_LINUX_ISOLATION_DOCKER_BINARY).mode;
  assert((mode & 0o111) !== 0, 'Docker binary is not executable');
