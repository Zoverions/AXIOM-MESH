...
function normalizeFactor(raw, index) {
  ...
  const effectiveAt = canonicalIso(raw.effective_at, `factor_profiles[${index}].effective_at`);
  const expiresAt = raw.expires_at === null
    ? null
    : canonicalIso(raw.expires_at, `factor_profiles[${index}].expires_at`);
  if (expiresAt !== null && expiresAt <= effectiveAt) {
    throw new ValidationError('recovery factor must expire after activation');
  }
  if (!['active', 'revoked', 'retired'].includes(raw.status)) {
    throw new ValidationError('recovery factor status is invalid');
  }
  return {
    factor_id: id(raw.factor_id, `factor_profiles[${index}].factor_id`),
    factor_type: factorType,
    factor_class: factorClass,
    attestor_actor_id: raw.attestor_actor_id === null
      ? null
      : id(raw.attestor_actor_id, `factor_profiles[${index}].attestor_actor_id`),
    minimum_assurance: assurance(
      raw.minimum_assurance,
      `factor_profiles[${index}].minimum_assurance`
    ),
...
export function normalizeRecoveryPolicy(raw) {
  exactKeys(raw, [
    ...
    'minimum_decision_assurance',
    'effective_at',
    'expires_at',
    'status',
    'ordinary_authority_granted'
  ], 'recovery policy');
  if (raw.schema !== RECOVERY_POLICY_SCHEMA) {
    throw new ValidationError('recovery policy schema is invalid');
  }
  ...
  if (!Array.isArray(raw.required_factor_classes) || raw.required_factor_classes.length > FACTOR_CLASSES.size) {
  ...
  const requiredClasses = raw.required_factor_classes.map((value, index) => {
    ...
    if (!FACTOR_CLASSES.has(factorClass)) {
      throw new ValidationError('required recovery factor class is invalid');
    }
    return factorClass;
  });
  ...
  const expiresAt = raw.expires_at === null ? null : canonicalIso(raw.expires_at, 'expires_at');
  if (expiresAt !== null && expiresAt <= effectiveAt) {
    throw new ValidationError('recovery policy must expire after activation');
  }
  if (!['active', 'revoked', 'retired'].includes(raw.status)) {
    throw new ValidationError('recovery policy status is invalid');
  }
  return {
    ...
    minimum_decision_assurance: assurance(
      raw.minimum_decision_assurance,
      'minimum_decision_assurance'
    ),
    effective_at: effectiveAt,
    expires_at: expiresAt,
...
export function normalizeRecoveryAttestation(raw) {
  ...
  if (raw.schema !== RECOVERY_ATTESTATION_SCHEMA) {
    throw new ValidationError('recovery attestation schema is invalid');
  }
  if (!['pass', 'fail'].includes(raw.verdict)) {
    throw new ValidationError('recovery attestation verdict is invalid');
  }
  ...
  const observedAt = canonicalIso(raw.observed_at, 'observed_at');
  const expiresAt = canonicalIso(raw.expires_at, 'expires_at');
  if (expiresAt <= observedAt) {
    throw new ValidationError('recovery attestation must expire after observation');
  }
  return {
    ...
    attestor_actor_id: raw.attestor_actor_id === null
      ? null
      : id(raw.attestor_actor_id, 'attestor_actor_id'),
...
export function evaluateRecovery(policyRaw, attestationsRaw, {
  ...
  if (policy.subject_actor_id !== subject) {
    throw new ValidationError('recovery policy subject mismatch');
  }
  if (
    policy.status !== 'active'
    || policy.effective_at > observedAt
    || (policy.expires_at && policy.expires_at <= observedAt)
  ) {
  ...
  for (const raw of attestationsRaw) {
    ...
    if (seen.has(attestation.factor_id)) {
      throw new ValidationError('duplicate recovery factor attestation');
    }
    ...
    if (attestation.expires_at <= observedAt || attestation.observed_at > observedAt) {
      reasons.push('attestation_not_current');
    }
    if (assuranceRank(attestation.assurance) < assuranceRank(policy.minimum_decision_assurance)) {
      reasons.push('decision_assurance_too_low');
    }
    if (profile) {
      if (
        profile.status !== 'active'
        || profile.effective_at > observedAt
        || (profile.expires_at && profile.expires_at <= observedAt)
      ) reasons.push('factor_profile_inactive');
      if (
        profile.factor_type !== attestation.factor_type
        || profile.factor_class !== attestation.factor_class
      ) reasons.push('factor_profile_mismatch');
      if (profile.attestor_actor_id !== attestation.attestor_actor_id) {
        reasons.push('attestor_mismatch');
      }
      if (assuranceRank(attestation.assurance) < assuranceRank(profile.minimum_assurance)) {
        reasons.push('factor_assurance_too_low');
      }
    }
  ...
  const achievedAssurance = accepted.reduce((minimum, item) => (
    minimum === null || assuranceRank(item.assurance) < assuranceRank(minimum)
      ? item.assurance
      : minimum
  ), null);
  const approved = accepted.length >= policy.threshold
    ...
    && assuranceRank(achievedAssurance) >= assuranceRank(policy.minimum_decision_assurance);
  return {
...
function appendRecoveredCredentialEpoch(actor, nextCredentialEpoch) {
  const next = normalizeCredentialEpoch(nextCredentialEpoch);
  if (next.actor_id !== actor.actor_id) {
    throw new ValidationError('recovery credential cannot change actor identity');
  }
  if (next.sequence !== actor.credential_epochs.length + 1 || next.state !== 'active') {
    throw new ValidationError('recovery credential must be the next active credential epoch');
  }
  const prior = actor.credential_epochs.at(-1);
  if (!prior || next.predecessor_epoch_id !== prior.epoch_id) {
    throw new ValidationError('recovery credential must bind the latest credential epoch');
  }
  const epochs = actor.credential_epochs.map((epoch, index) => {
    if (index !== actor.credential_epochs.length - 1 || epoch.state !== 'active') return epoch;
    return { ...epoch, state: 'retired', ended_at: next.activated_at };
  });
  epochs.push(next);
...
export function completeRecovery({ actorState, decision, nextCredentialEpoch, completedAt }) {
  ...
  if (decided.subject_actor_id !== actor.actor_id) {
    throw new ValidationError('recovery decision actor mismatch');
  }
  ...
  if (decided.ordinary_authority_granted !== false) {
    throw new ValidationError('recovery decision cannot grant ordinary authority');
  }
  const state = appendRecoveredCredentialEpoch(actor, nextCredentialEpoch);
  const completed = canonicalIso(completedAt, 'completedAt');
  return {
    schema: RECOVERY_COMPLETION_SCHEMA,
    recovery_case_id: id(decided.recovery_case_id, 'decision.recovery_case_id'),
    subject_actor_id: actor.actor_id,
    prior_credential_epoch_id: actor.credential_epochs.at(-1)?.epoch_id ?? null,
    ...
    assurance: assurance(decided.achieved_assurance, 'decision.achieved_assurance'),
