...
function uniqueIds(values, name, maxItems = 64) {
  const items = assertStringArray(values, name, { maxItems, itemMax: 192 }).map((value, index) => id(value, `${name}[${index}]`));
  if (new Set(items).size !== items.length) throw new ValidationError(`${name} must be unique`);
  return items.sort();
}

export function normalizeStateAccessEnvelope(raw) {
  exactKeys(raw, [
    'schema', 'envelope_id', 'subject_actor_id', 'requester_actor_id', 'state_class',
    'purpose', 'action', 'data_scopes', 'recipient_actor_ids', 'disclosure_profile',
    'authority_basis', 'consent', 'required_assurance', 'observed_assurance',
    'effective_at', 'expires_at', 'raw_state_allowed', 'grants_ordinary_authority'
  ], 'state access envelope');
  if (raw.schema !== STATE_ACCESS_ENVELOPE_SCHEMA) throw new ValidationError('state access envelope schema is invalid');
  ...
  if (typeof consent.required !== 'boolean') throw new ValidationError('consent.required must be boolean');
  const receiptDigest = consent.receipt_digest === null ? null : digest(consent.receipt_digest, 'consent.receipt_digest');
  if (consent.required && receiptDigest === null) throw new ValidationError('required consent must bind a receipt digest');
  if (!consent.required && receiptDigest !== null) throw new ValidationError('non-required consent cannot imply a receipt binding');
  ...
  if (ASSURANCE[observedAssurance] < ASSURANCE[requiredAssurance]) {
  ...
  if (typeof raw.raw_state_allowed !== 'boolean') throw new ValidationError('raw_state_allowed must be boolean');
  ...
  if (action === 'publish' && disclosure !== 'public' && disclosure !== 'pseudonymous') {
  ...
  if (basis.type === 'association_obligation' && action !== 'contribute_metric' && action !== 'export') {
  ...
  const effectiveAt = iso(raw.effective_at, 'effective_at');
  const expiresAt = iso(raw.expires_at, 'expires_at');
  if (expiresAt <= effectiveAt) throw new ValidationError('state access envelope must expire after activation');
  return {
    ...
    consent: { required: consent.required, receipt_digest: receiptDigest },
...
export function verifyStateAccessUse(envelopeRaw, useRaw, now = new Date().toISOString()) {
  ...
  exactKeys(useRaw, [
    'subject_actor_id', 'requester_actor_id', 'state_class', 'purpose', 'action',
    'data_scopes', 'recipient_actor_ids', 'disclosure_profile', 'payload_digest'
  ], 'state access use');
  for (const field of ['subject_actor_id', 'requester_actor_id', 'state_class', 'purpose', 'action', 'disclosure_profile']) {
    if (useRaw[field] !== envelope[field]) throw new ValidationError(`${field} does not match the authorized envelope`);
  }
  exactSet(uniqueIds(useRaw.data_scopes, 'use.data_scopes'), envelope.data_scopes, 'data_scopes');
  exactSet(uniqueIds(useRaw.recipient_actor_ids, 'use.recipient_actor_ids', 32), envelope.recipient_actor_ids, 'recipient_actor_ids');
