...
export const INTENT_EXECUTOR_PROMOTION_PACKAGE_SCHEMA = 'axiom-intent-executor-promotion-package.v1';
export const INTENT_EXECUTOR_REGISTRY_PATH = 'mesh/config/intent-remediation-executors.json';
export const INTENT_EXECUTOR_REGISTRY_REPOSITORY = 'Zoverions/AXIOM-MESH';
...
function stripComputedMappingFields(mapping) {
  ...
  const {
    mapping_digest: ignoredMappingDigest,
    ...documentMapping
  } = value;
...
function assertOneAddition(baseRegistry, nextRegistry, candidateMapping) {
  if (nextRegistry.mappings.length !== baseRegistry.mappings.length + 1) {
    throw new ValidationError('proposed registry must contain exactly one added mapping');
  }
  ...
  for (const [semanticAction, mapping] of base) {
    ...
    if (!proposed || canonicalJson(proposed) !== canonicalJson(mapping)) {
      throw new ValidationError('proposed registry may not delete or modify an existing mapping');
    }
  }
  if (base.has(candidateMapping.semantic_action)) {
    throw new ValidationError('promotion candidate semantic action already exists in the base registry');
  }
  const added = next.get(candidateMapping.semantic_action);
  if (!added || canonicalJson(added) !== canonicalJson(candidateMapping)) {
    throw new ValidationError('proposed registry addition does not exactly match the reviewed candidate mapping');
  }
}

function reviewBindings(candidate) {
  if (!Array.isArray(candidate.reviews) || candidate.reviews.length !== 2) {
    throw new ValidationError('promotion candidate must bind exactly two independent reviews');
  }
...
function buildPackageBody({ candidate, base, proposed }) {
  return {
    schema: INTENT_EXECUTOR_PROMOTION_PACKAGE_SCHEMA,
    ...
    verification: {
      schema: 'axiom-intent-executor-promotion-package-verification.v1',
      required_checks: [
        'verify_v0_7_candidate_against_current_context',
        'verify_base_registry_file_bytes_digest',
        'verify_base_registry_semantic_digest',
        'verify_exactly_one_mapping_addition',
    ...
    non_claim: 'This offline package describes an exact reviewed registry change. It does not apply, install, approve, or execute that change.'
  };
}

export function buildIntentExecutorPromotionPackage({
  promotion_candidate,
  dossier,
  reviews,
  current_context,
  ...
  if (base.normalized.registry_digest !== contextRegistry.registry_digest) {
    throw new ValidationError('current registry file bytes do not match the current semantic executor registry');
  }
  if (candidate.expected_executor_registry_digest !== base.normalized.registry_digest) {
    throw new ValidationError('promotion candidate expects a different base executor registry');
  }
  if (base.normalized.mappings.some(mapping => mapping.semantic_action === candidate.mapping.semantic_action)) {
    throw new ValidationError('promotion candidate semantic action already exists in the current registry');
  }
  ...
  return {
    ...body,
    package_id: `intent-executor-promotion-package:${packageDigest}`,
    package_digest: packageDigest
  };
}

export function normalizeIntentExecutorPromotionPackage(raw) {
  const value = assertPlainObject(raw, 'Intent executor promotion package');
  if (value.schema !== INTENT_EXECUTOR_PROMOTION_PACKAGE_SCHEMA) {
    throw new ValidationError(`promotion package schema must be ${INTENT_EXECUTOR_PROMOTION_PACKAGE_SCHEMA}`);
  }
  if (
    ...
    throw new ValidationError('promotion package must remain non-applying, non-installing, and non-executing');
  }
  ...
  if (packageDigest !== expectedDigest || packageId !== `intent-executor-promotion-package:${expectedDigest}`) {
    throw new ValidationError('executor promotion package is not content-addressed');
  }
  return {
...
export function verifyIntentExecutorPromotionPackage(rawPackage, {
  promotion_candidate,
  dossier,
  reviews,
  current_context,
  ...
  const supplied = normalizeIntentExecutorPromotionPackage(rawPackage);
  const expected = buildIntentExecutorPromotionPackage({
    promotion_candidate,
    dossier,
    reviews,
    current_context,
  ...
  if (canonicalJson(supplied) !== canonicalJson(expected)) {
    throw new ValidationError('executor promotion package does not match the exact current reviewed promotion state');
  }
  const proposed = parseRegistryBytes(supplied.proposed_registry.utf8, 'package.proposed_registry.utf8');
  if (
    ...
    throw new ValidationError('executor promotion package proposed registry checksums are invalid');
  }
  const base = parseRegistryBytes(current_registry_bytes, 'current_registry_bytes');
