...
import {
  ...
  verifyResearchClaimAdjudicationBinding
...
function validateVerifiers(verifiers, claimedIndependentConfirmations) {
  ...
  const uniqueSupportingDigests = new Map();
  ...
  verifiers.forEach((verifier, index) => {
    ...
    if (seenIds.has(verifier.verifier_id)) {
      throw new ValidationError(`verifiers contains duplicate verifier_id ${verifier.verifier_id}`);
    }
    seenIds.add(verifier.verifier_id);

    const priorGroup = digestGroups.get(verifier.verifier_digest);
    if (priorGroup !== undefined && priorGroup !== verifier.independence_group) {
      throw new ValidationError(
        `verifiers digest ${verifier.verifier_digest} cannot claim multiple independence groups`
      );
    }
    digestGroups.set(verifier.verifier_digest, verifier.independence_group);

    if (verifier.result === 'supports' && !uniqueSupportingDigests.has(verifier.verifier_digest)) {
      uniqueSupportingDigests.set(verifier.verifier_digest, verifier.independence_group);
    }
  });

  const supportingGroups = new Set(uniqueSupportingDigests.values());
  ...
  if (uniqueSupportingDigests.size > 0 && supportingGroups.size === 0) {
    throw new ValidationError('supporting verifiers must declare an independence group');
  }
}

const RESEARCH_ADJUDICATION_DIGEST_PATTERN = /^sha256:[0-9a-f]{64}$/;
const ADVERSE_CLAIM_STATUSES = new Set(['contested', 'unsupported', 'corrected']);
const ADVERSE_ALLOWED_INTERPRETATIONS = new Set([
  'conflicting-evidence',
  'supported-with-caveats',
  'insufficient-evidence'
]);

function assertClaimAdjudicationDigest(value, name) {
  if (value === null) return null;
  assertString(value, name, { min: 71, max: 71, pattern: RESEARCH_ADJUDICATION_DIGEST_PATTERN });
  return value;
}

function applyClaimAdjudication(status, reasonCodes, unresolvedUnknowns, adjudication) {
  const codes = [...reasonCodes];
  const unknowns = [...unresolvedUnknowns];
  let nextStatus = status;

  if (adjudication.status === 'supported') {
    if (nextStatus === 'supported' || nextStatus === 'supported-with-caveats') {
      codes.push('claim-adjudication-supported');
    }
    return { status: nextStatus, reasonCodes: codes, unresolvedUnknowns: unknowns };
  }

  if (adjudication.status === 'insufficient_evidence') {
    if (nextStatus === 'supported') {
      nextStatus = 'insufficient-evidence';
    } else if (nextStatus === 'supported-with-caveats') {
      nextStatus = 'insufficient-evidence';
    }
    codes.push('claim-adjudication-insufficient-evidence');
    unknowns.push('claim-adjudication-insufficient-evidence');
    return { status: nextStatus, reasonCodes: codes, unresolvedUnknowns: unknowns };
  }

  if (ADVERSE_CLAIM_STATUSES.has(adjudication.status)) {
    if (adjudication.status === 'contested') {
      nextStatus = 'conflicting-evidence';
      codes.push('claim-adjudication-contested');
    } else if (adjudication.status === 'unsupported') {
      nextStatus = nextStatus === 'conflicting-evidence' ? 'conflicting-evidence' : 'insufficient-evidence';
      codes.push('claim-adjudication-unsupported');
    } else {
      // corrected
      nextStatus = nextStatus === 'conflicting-evidence' ? 'conflicting-evidence' : 'supported-with-caveats';
      codes.push('claim-adjudication-corrected');
      unknowns.push('claim-adjudication-corrected');
    }
    if (nextStatus === 'supported' || !ADVERSE_ALLOWED_INTERPRETATIONS.has(nextStatus)) {
      nextStatus = 'conflicting-evidence';
    }
    return { status: nextStatus, reasonCodes: codes, unresolvedUnknowns: unknowns };
  }

  throw new ValidationError('claim adjudication status is invalid');
}
...
function validateSemantics(document, { verifyDigest = true } = {}) {
  ...
  assertClaimAdjudicationDigest(document.claim_adjudication_digest, 'claim_adjudication_digest');
...
function resolvedEnvelope(document, {
  ...
  reasonCodes = [],
  unresolvedUnknowns = null,
  claimAdjudicationStatus = null
}) {
  return deepFreeze({
    valid: true,
    ...
    unresolved_unknowns: [...(unresolvedUnknowns ?? document.unresolved_unknowns)],
    claim_adjudication_digest: document.claim_adjudication_digest,
    claim_adjudication_status: claimAdjudicationStatus,
    observed_at: document.observed_at,
    authority_effect: 'none',
    network_effect: 'none',
    runtime_activation: false,
...
function finalizeResolvedEnvelope(document, fields, {
  claimAdjudication = null,
  knowledgeProjection = null,
  sourceManifest = null
} = {}) {
  let status = fields.status;
  let reasonCodes = [...(fields.reasonCodes || [])];
  let unresolvedUnknowns = [...document.unresolved_unknowns];
  let claimAdjudicationStatus = null;

  if (document.claim_adjudication_digest !== null) {
    if (claimAdjudication === null || claimAdjudication === undefined) {
      return resolvedEnvelope(document, {
        ...fields,
        status: 'invalid-evidence',
        reasonCodes: [...reasonCodes, 'claim-adjudication-missing'],
        unresolvedUnknowns,
        claimAdjudicationStatus: null
      });
    }
    const verified = (knowledgeProjection !== null && sourceManifest !== null)
      ? verifyResearchClaimAdjudicationBinding(claimAdjudication, knowledgeProjection, sourceManifest)
      : verifyResearchClaimAdjudication(claimAdjudication);
    if (verified.adjudication_digest !== document.claim_adjudication_digest) {
      return resolvedEnvelope(document, {
        ...fields,
        status: 'invalid-evidence',
        reasonCodes: [...reasonCodes, 'claim-adjudication-digest-mismatch'],
        unresolvedUnknowns,
        claimAdjudicationStatus: null
      });
    }
    claimAdjudicationStatus = verified.status;
    const adjusted = applyClaimAdjudication(status, reasonCodes, unresolvedUnknowns, verified);
    status = adjusted.status;
    reasonCodes = adjusted.reasonCodes;
    unresolvedUnknowns = adjusted.unresolvedUnknowns;
    if (status === 'supported' && ADVERSE_CLAIM_STATUSES.has(verified.status)) {
...
export function resolveResponseAssuranceEnvelope(document, profile, {
  now,
  calibrationReports = undefined,
  claimAdjudication = null,
  knowledgeProjection = null,
  sourceManifest = null
} = {}) {
  validateResponseAssuranceEnvelope(document);
  validateBehavioralAssuranceProfile(profile, { calibrationReports, now });
  ...
  const uniqueSupportingDigests = new Map();
  for (const item of document.verifiers) {
    if (item.result !== 'supports') continue;
    if (!uniqueSupportingDigests.has(item.verifier_digest)) {
      uniqueSupportingDigests.set(item.verifier_digest, item.independence_group);
    }
  }
  const independentConfirmationCount = new Set(uniqueSupportingDigests.values()).size;
  ...
  if (
    ...
    return finalizeResolvedEnvelope(document, {
      status: 'invalid-evidence',
      profileApplicability: 'incompatible',
      deterministicFailureCount,
      independentConfirmationCount,
      reasonCodes: ['profile-binding-mismatch']
    }, { claimAdjudication, knowledgeProjection, sourceManifest });
  }
