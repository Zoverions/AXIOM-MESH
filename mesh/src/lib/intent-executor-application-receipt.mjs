...
import { normalizeIntentExecutorAdmissionDossier } from './intent-executor-admission.mjs';
import {
...
export const INTENT_EXECUTOR_APPLICATION_RECEIPT_SCHEMA = 'axiom-intent-executor-application-receipt.v1';
...
function assertExactOneAddition(preRegistry, postRegistry, mapping) {
  if (postRegistry.mappings.length !== preRegistry.mappings.length + 1) {
    throw new ValidationError('observed post registry must contain exactly one added mapping');
  }
  ...
  for (const [semanticAction, existing] of pre) {
    ...
    if (!observed || canonicalJson(observed) !== canonicalJson(existing)) {
      throw new ValidationError('observed post registry modified or deleted an existing mapping');
    }
  }
  if (pre.has(mapping.semantic_action)) {
    throw new ValidationError('candidate semantic action already existed before application');
  }
  const addition = post.get(mapping.semantic_action);
  if (!addition || canonicalJson(addition) !== canonicalJson(mapping)) {
    throw new ValidationError('observed post registry does not contain the exact reviewed mapping addition');
  }
}
...
function receiptBodyFrom(raw) {
  const value = assertPlainObject(raw, 'Intent executor application receipt');
  if (value.schema !== INTENT_EXECUTOR_APPLICATION_RECEIPT_SCHEMA) {
    throw new ValidationError(`application receipt schema must be ${INTENT_EXECUTOR_APPLICATION_RECEIPT_SCHEMA}`);
  }
  if (
    ...
    throw new ValidationError('application receipt observation/authority flags are invalid');
  }
  ...
  if (
    ...
    throw new ValidationError('application receipt destination is invalid');
  }
  ...
  if (!Array.isArray(value.reviews) || value.reviews.length !== 2) {
    throw new ValidationError('application receipt must bind exactly two mapping reviews');
  }
  ...
  return {
    schema: INTENT_EXECUTOR_APPLICATION_RECEIPT_SCHEMA,
    ...
    remediation_execution_observed: false,
...
export function buildIntentExecutorApplicationReceipt({
  promotion_package,
  promotion_candidate,
  dossier: rawDossier,
  reviews,
  ...
  if (!identity || typeof identity.signObject !== 'function' || typeof identity.service !== 'string') {
    throw new ValidationError('application receipt requires an AXIOM signing identity');
  }
  if (application_verifier !== identity.service) {
    throw new ValidationError('application verifier must match the signing identity service');
  }
  ...
  if (forbiddenVerifierIdentities(dossier, promotionPackage).has(application_verifier)) {
    throw new ValidationError('application verifier must be independent from the dossier producer and mapping reviewers');
  }
  ...
  if (
    ...
    throw new ValidationError('pre-application registry does not exactly match the reviewed package base');
  }
  if (post.bytes !== promotionPackage.proposed_registry.utf8) {
    throw new ValidationError('post-application registry bytes do not exactly match the reviewed package proposal');
  }
  if (
    ...
    throw new ValidationError('post-application registry checksums do not match the reviewed package proposal');
  }
  ...
  const body = receiptBodyFrom({
    ...
    execution_authorized: false,
    remediation_execution_observed: false,
    non_claim: 'This signed receipt observes an exact external registry application. It does not authorize application, grant a capability, or prove any remediation effect executed.'
  });
  ...
  return {
    ...body,
    signature,
    receipt_id: `intent-executor-application-receipt:${receiptDigest}`,
    receipt_digest: receiptDigest
  };
}

export function verifyIntentExecutorApplicationReceipt(rawReceipt, {
  promotion_package,
  promotion_candidate,
  dossier: rawDossier,
  reviews,
  ...
  const value = assertPlainObject(rawReceipt, 'Intent executor application receipt');
  ...
  if (
    ...
    || value.receipt_id !== `intent-executor-application-receipt:${receiptDigest}`
  ) {
    throw new ValidationError('application receipt is not content-addressed');
  }
  if (!signature.key_id?.startsWith(`${body.application_verifier}:`)) {
    throw new ValidationError('application receipt key id does not match verifier identity');
  }
  if (!public_key || !verifyObjectSignature(body, signature, public_key)) {
    throw new ValidationError('application receipt signature is invalid');
  }
  ...
  const expected = buildIntentExecutorApplicationReceipt({
    promotion_package,
    promotion_candidate,
    dossier: rawDossier,
    reviews,
  ...
  const { signature: ignoredExpectedSignature, receipt_id: ignoredExpectedId, receipt_digest: ignoredExpectedDigest, ...expectedBody } = expected;
  if (canonicalJson(body) !== canonicalJson(expectedBody)) {
    throw new ValidationError('application receipt does not match the exact current package and observed registry state');
  }
  ...
  if (new Date(body.verified_at).valueOf() > nowDate.valueOf() + 5 * 60 * 1000) {
    throw new ValidationError('application receipt verification timestamp is from the future');
  }
  return {
    ...body,
    signature,
