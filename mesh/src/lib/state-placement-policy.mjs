...
export function evaluateStatePlacement({ request, policy, destinations, now }) {
  ...
  for (const destination of verifiedDestinations) {
    const reasonCodes = evaluateDestination({
  ...
  const requestExpiry = parseTime(verifiedRequest.expires_at, 'request.expires_at');
  const policyExpiry = parseTime(verifiedPolicy.expires_at, 'policy.expires_at');
...
function evaluateDestination({ request, policy, destination }) {
  ...
  if (!destination.allowed_operations.includes(request.operation_class)) {
    reasons.add('operation-unsupported');
  }
  if (!destination.allowed_purposes.includes(request.purpose)) {
    reasons.add('purpose-unsupported');
  }
  if (!destination.allowed_data_classes.includes(request.data_class)) {
    reasons.add('data-class-unsupported');
  }
  if (CONFIDENTIALITY_RANK[destination.maximum_confidentiality] < CONFIDENTIALITY_RANK[request.confidentiality_requirement]) {
    reasons.add('confidentiality-insufficient');
  }
  if (!destination.disclosure_modes.includes(request.disclosure_ceiling)) {
    reasons.add('disclosure-mode-unsupported');
  }
  if (!destination.regions.some((region) => request.residency.allowed_regions.includes(region))) {
    reasons.add('residency-region-mismatch');
  }
  ...
  if (RESIDENCY_EVIDENCE_RANK[destination.residency_evidence_level] < requiredResidencyEvidence) {
    reasons.add('residency-evidence-insufficient');
  }
  if (
    destination.retention_days.minimum > request.retention.minimum_days ||
    destination.retention_days.maximum < request.retention.maximum_days
  ) {
    reasons.add('retention-window-unsupported');
  }
  if (destination.maximum_lag_ms > request.maximum_lag_ms) {
    reasons.add('freshness-unsupported');
  }
  if (!destination.consistency_classes.includes(request.consistency_class)) {
    reasons.add('consistency-unsupported');
  }
  if (!destination.encryption_profiles.includes(request.encryption_profile)) {
    reasons.add('encryption-profile-unsupported');
  }
  if (!destination.recovery_importance_supported.includes(request.recovery_importance)) {
    reasons.add('recovery-importance-unsupported');
  }
  if (destination.cost_units > request.cost_ceiling_units) {
...
function rejectDuplicateDestinationIdentity(destinations) {
  ...
  for (const destination of destinations) {
    if (ids.has(destination.destination_id)) {
      throw new ValidationError(`duplicate destination_id ${destination.destination_id}`);
    }
