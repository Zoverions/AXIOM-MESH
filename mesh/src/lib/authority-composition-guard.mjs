...
function stringSequence(raw, label, { minItems = 0, maxItems = 128 } = {}) {
  if (!Array.isArray(raw) || raw.length < minItems || raw.length > maxItems) {
    throw new ValidationError(
      `${label} must be an array of ${minItems}-${maxItems} ordered strings`
    );
  }
...
function normalizeRestrictions(raw) {
  ...
  return Object.freeze(raw.map((entry, index) => {
    ...
    const actions = stringSequence(
      value.ordered_actions,
      `composition restrictions[${index}].ordered_actions`,
...
export function evaluateAuthorityComposition({
  grant,
  intent,
  request,
  history = [],
  ...
  const sameCausalHistory = normalizedHistory.filter(
    entry => entry.causal_scope_id === normalizedRequest.causal_scope_id
  );
