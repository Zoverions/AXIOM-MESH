...
function validateEvidence(value) {
  ...
  if (value.assurance_class === 'declared') {
    if (hasVerificationRef) {
      throw new ValidationError('declared assurance cannot claim verification evidence');
    }
  } else if (!hasVerificationRef) {
    throw new ValidationError(`${value.assurance_class} assurance requires verification evidence`);
  }
  ...
  if (
    ...
    throw new ValidationError('signed-evaluation-run cannot use declared assurance');
  }
}
