...
function digestArray(raw, label, maxItems = 64) {
  ...
  if (unique.length !== values.length) throw new ValidationError(`${label} must not contain duplicates`);
...
function normalizeOrigin(raw) {
  ...
  return Object.freeze({
    ...
    lineage_record_digest: lineageDigest
  });
}
...
function normalizeResources(raw) {
  const value = exactObject(raw, RESOURCE_KEYS, 'improvement proposal resources');
  return Object.freeze({
    max_evaluation_runs: safeInteger(
      value.max_evaluation_runs,
      'improvement proposal resources max_evaluation_runs',
      { min: 1, max: 100_000 }
    ),
    max_child_agents: safeInteger(
      value.max_child_agents,
      'improvement proposal resources max_child_agents',
      { min: 1, max: 10_000 }
    ),
    max_cost_units: safeInteger(
      value.max_cost_units,
      'improvement proposal resources max_cost_units',
      { min: 1, max: 1_000_000_000_000 }
    ),
    max_wall_clock_ms: safeInteger(
      value.max_wall_clock_ms,
      'improvement proposal resources max_wall_clock_ms',
      { min: 1, max: 2_678_400_000 }
    ),
    max_storage_bytes: safeInteger(
      value.max_storage_bytes,
      'improvement proposal resources max_storage_bytes',
      { min: 1, max: 1_125_899_906_842_624 }
    )
  });
}
...
function minimumConsequence(surface) {
  return SURFACE_MINIMUM[surface];
}
...
export function normalizeImprovementProposal(raw, { now } = {}) {
  ...
  const targetSurface = enumValue(value.target_surface, 'improvement proposal target_surface', TARGET_SURFACES);
  const consequenceClass = enumValue(
    value.consequence_class,
    'improvement proposal consequence_class',
    new Set(CONSEQUENCE_CLASSES)
  );
  const minimum = minimumConsequence(targetSurface);
  if (CONSEQUENCE_INDEX.get(consequenceClass) < CONSEQUENCE_INDEX.get(minimum)) {
    throw new ValidationError(
      `improvement proposal minimum consequence for ${targetSurface} is ${minimum}`
    );
  }
  ...
  return Object.freeze({
    ...normalized,
    proposal_digest: digestObject(normalized)
  });
}
...
export function normalizeImprovementEvaluation(raw, {
  ...
  return Object.freeze({
    ...normalized,
    evaluation_digest: digestObject(normalized)
  });
}
...
function summarizeEvaluations(evaluations) {
  const positive = evaluations.filter(item => item.verdict === 'positive').length;
  const negative = evaluations.filter(item => item.verdict === 'negative').length;
  const mixed = evaluations.filter(item => item.verdict === 'mixed').length;
  ...
  return Object.freeze({
    ...
    negative_evaluations: negative,
    mixed_evaluations: mixed,
...
export function createImprovementExperiment(raw = {}) {
  ...
  return Object.freeze({
    ...normalized,
    experiment_digest: experimentDigest
  });
}
...
function assessmentRequirements(consequenceClass) {
...
export function assessImprovementPromotion(raw = {}) {
  ...
  if (args.profileOverrides !== undefined && args.profileOverrides !== null) {
    throw new ValidationError('improvement promotion profileOverrides are not implemented in v0 core');
  }
  const experiment = revalidateExperiment(args.experiment);
  const proposal = experiment.proposal;
  const requirements = assessmentRequirements(proposal.consequence_class);
  ...
  if (proposal.consequence_class === 'C1') {
  ...
  } else if (CONSEQUENCE_INDEX.get(proposal.consequence_class) >= 2) {
    if (
  ...
  if (proposal.target_surface === 'evaluator-reward') {
  ...
  return Object.freeze({
    ...normalized,
    assessment_digest: digestObject(normalized)
  });
}
