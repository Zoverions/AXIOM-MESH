...
export function defineAssertionLadder(definition) {
  requireCondition(
    ...
    'INVALID_LADDER',
    'ladder must be an object'
  );
  requireCondition(
    definition.schema === ASSERTION_LADDER_SCHEMA,
    'INVALID_SCHEMA',
    'unsupported assertion-ladder schema'
  );
  requireCondition(
    typeof definition.ladder_id === 'string' && definition.ladder_id.length > 0,
    'INVALID_LADDER_ID',
    'ladder_id is required'
  );
  requireCondition(
    Array.isArray(definition.states) && definition.states.length > 0,
    'INVALID_STATES',
  ...
  const states = definition.states.map((state, index) => {
    requireCondition(
      ...
      'INVALID_STATE',
      `state ${index} must be an object`
    );
    requireCondition(
      typeof state.id === 'string' && state.id.length > 0,
      'INVALID_STATE_ID',
      `state ${index} id is required`
    );
    requireCondition(
      Number.isInteger(state.rank) && state.rank >= 0,
      'INVALID_STATE_RANK',
      `${state.id} rank must be a non-negative integer`
    );
  ...
  states.forEach((state, index) => {
    requireCondition(
      ...
      'state ranks must be contiguous starting at zero'
    );
  });
  ...
  for (const failureState of failureStates) {
    requireCondition(
      typeof failureState === 'string' && failureState.length > 0,
      'INVALID_FAILURE_STATE',
      'failure states must be non-empty strings'
    );
    requireCondition(
      !ids.has(failureState),
      'FAILURE_STATE_COLLISION',
      `${failureState} cannot be both ranked and failure state`
    );
  }
...
export function describeAssertion(ladder, stateId) {
  ...
  if (state) {
    return {
      kind: 'ranked',
      state,
      not_yet: ladder.states
        .filter(candidate => candidate.rank > state.rank)
  ...
  if (ladder.failure_states.includes(stateId)) {
    return {
      kind: 'failure',
      state: { id: stateId, terminal: true },
      not_yet: ladder.states.map(candidate => candidate.id)
    };
  }
  throw new AssertionLadderError(
    'UNKNOWN_STATE',
    `unknown state ${stateId} for ${ladder.ladder_id}`
  );
}
...
export function createAssertionEvent({
  ...
  requireCondition(
    typeof subject_id === 'string' && subject_id.length > 0,
    'INVALID_SUBJECT',
  ...
  if (previous) {
    requireCondition(
      previous.ladder_id === ladder.ladder_id,
      'LADDER_MISMATCH',
      'previous event belongs to another ladder'
    );
    requireCondition(
      previous.subject_id === subject_id,
      'SUBJECT_MISMATCH',
      'previous event belongs to another subject'
    );
    ...
    if (described.kind === 'ranked') {
      requireCondition(
        described.state.rank > prior.state.rank,
        'NON_MONOTONIC_TRANSITION',
        `transition ${previous.state} -> ${state} is not an upgrade`
      );
    }
  }
...
export function resolvePolicyDeclaredTier({
  policy_tier,
  caller_tier = null,
  allowed_tiers
}) {
  requireCondition(
    Array.isArray(allowed_tiers) && allowed_tiers.includes(policy_tier),
    'INVALID_POLICY_TIER',
    'policy tier is not allowed'
  );
  requireCondition(
    caller_tier === null || caller_tier === policy_tier,
    'CALLER_TIER_OVERRIDE_DENIED',
    'caller may not choose or downgrade the policy-declared assurance tier'
  );
  return policy_tier;
}
