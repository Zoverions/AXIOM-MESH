...
function ids(value, label, { min = 0, max = 128 } = {}) {
  if (!Array.isArray(value) || value.length < min || value.length > max) {
    throw new ValidationError(`${label} must contain ${min}-${max} identifiers`);
  }
  const out = value.map((entry, index) => id(entry, `${label}[${index}]`));
  if (new Set(out).size !== out.length) throw new ValidationError(`${label} must be unique`);
  return Object.freeze(out);
}
...
export function validateStudioArtifactManifest(raw) {
  ...
  ids(value.protection_profile_ids, 'Studio artifact protection_profile_ids', { min: 1 });
  ids(value.verification_profile_ids, 'Studio artifact verification_profile_ids', { min: 1 });
  ids(value.deployment_topology_compatibility, 'Studio artifact deployment_topology_compatibility', { min: 1 });
