...
/**
 * Produce the canonical identity of a requested effect before transient fields
 * such as intent_id, submitted_at, confirmations, or approval IDs are added.
 *
 * Gateway idempotency and Hypervisor approval binding MUST use this exact
 * helper so transport/retry code cannot drift from authorization identity.
 */
export function intentRequestBinding(intent) {
  ...
  return {
    ...
    ...(machineAuthorityDigest ? { machine_authority_digest: machineAuthorityDigest } : {})
  };
}
