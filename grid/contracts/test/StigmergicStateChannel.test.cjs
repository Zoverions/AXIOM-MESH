...
describe("StigmergicStateChannel v4 interface audit", function () {
  ...
  it("settles and emits funding split once challenge window closes", async function () {
    ...
    expect(await axm.balanceOf(pool.address)).to.equal(ethers.parseEther("10"));
    expect(await axm.balanceOf(agentA.address)).to.equal(ethers.parseEther("45"));
    expect(await axm.balanceOf(agentB.address)).to.equal(ethers.parseEther("45"));
  });
});
