...
| SELF-REPORT-001 | In progress — docs/local-regression slice | Preserve substrate-neutral self-report evidence without authority promotion | [#1753](https://github.com/Zoverions/AXIOM-MESH/issues/1753); assertion/denial/uncertainty stay evidence-only; strict assurance input rejects self-report-shaped authority fields; valid machine authority and authenticated human consent/revocation remain governed by existing consumers; local synthetic regressions + architecture clarification; no consciousness detector, provider call, persistence, credential, network, or authority/capability promotion |
