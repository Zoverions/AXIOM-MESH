...
| CIRCLE-002 | In progress; inert shared-artifact mutation admission v0 | Shared objects/proposals/tasks/commitments/approvals/conflicts | Canonical Shared Artifact + Circle Core membership-assurance composition; exact Circle domain, actor/member, prior/candidate/artifact/authorization evidence binding; one append-only revision; stale-parent conflict remains visible; complete resolution remains canonical-artifact enforced; no write/runtime authority. Commitments, Circle approval records, persistence, multi-node apply/export/exit remain pending |
