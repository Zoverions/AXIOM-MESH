...
- [ ] **P0** M20.1 Cosmos IBC integration brief: define AXIOM augmentation narrative for Cosmos chains (ZKML verification, policy orchestration, fail-closed evidence paths).
- [ ] **P0** M20.2 Build Grid IBC-Go module spike (sender/receiver + light-client boundary and relayer assumptions).
- [ ] **P0** M20.3 Add Hypervisor IBC packet-channel design (`capability` routing, signed packet metadata, and provenance capture).
- [ ] **P0** M20.4 Specify ZKML attestation envelope for third-party compute outputs via IBC memo/payload transport with Grid verifier acceptance criteria.
- [ ] **P1** M20.5 Draft Akash bidirectional adapter plan (GPU job routing + result proof handoff + settlement checkpoints).
- [ ] **P1** M20.6 Produce Cosmos compatibility checklist (IBC v2 / ibc-go v10+ / ICS dependencies) and phased rollout gates.
- [ ] **P0** M20.7 Finalize Render partnership narrative focused on AXIOM augmentation and pilot scope.
- [ ] **P1** M20.8 Build Hypervisor Render adapter spike (external GPU job submission + callback ingestion + signed evidence hooks).
- [ ] **P1** M20.9 Draft Polkadot integration plan covering XCM sender/receiver boundaries, trust assumptions, and staged activation gates.
- [ ] **P1** M20.10 Define parachain readiness workstream (Agile Coretime strategy, treasury/governance impacts, and rollout prerequisites).
- [ ] **P1** M20.11 Add distributed cache design track (Redis/Dragonfly primary + persistent fallback) across Hypervisor, Grid, and Sandbox, including IBC/XCM packet state caching.
- [ ] **P1** M20.12 Extend tokenomics modeling for cross-network execution (burn/mint flows, staking/slashing, external compute settlement).
- [ ] **P1** M20.13 Add governance-closure modeling tasks for evidence-weighted referenda and partnership-risk controls.
- [ ] **P2** M20.14 Prepare partnership pitch assets (Cosmos IBC + Render + Polkadot augmentation matrix, pilot scope, and success metrics).
