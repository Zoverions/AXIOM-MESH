...
| PILOT-008 | Complete for evidence intake; authentic package pending | Require one build-bound signed pilot package without allowing either verifier to promote production | Separately pinned policy authority, five distinct review roles, exact canonical policy/dossier/13-envelope inventory, raw-byte digest and build binding, secret/symlink/unexpected-file rejection, role signatures, chronology checks, signed CI conformance evidence, and explicit `production_promoted: false` |
