...
| DOC-003 | In progress; initial high-risk policy/checker implemented | Make implementation changes carry required documentation/downstream review | Machine-readable documentation-impact rules cover host install, Education-consumed Mesh surfaces, service-network policy, capability status, application catalogue, and the policy itself; adversarial tests exist; protected base-to-head PR enforcement is being wired in this convergence slice; semantic completeness and broader ownership mapping remain future work |
...
| ID | Status | Work | Acceptance evidence |
|---|---|---|---|
| INSTALL-001 | In progress; non-mutating planner implemented | Fresh Linux personal/local node installer | Current planner validates the specified personal/local target, records explicit host blockers, binds host/policy/profile/target digests, preserves no-public-ingress/deny-egress/no-enrollment/no-authority, and performs no mutation; remaining acceptance requires a supported clean Linux host with no AXIOM checkout, immutable signed release verification, pinned host/runtime dependency acquisition, unprivileged service/data/secret boundaries, readiness, reboot, update, tampered/incompatible-update rejection, rollback-or-restore, second-host restore, uninstall, and non-secret install receipts |
| INSTALL-002 | In progress; non-mutating planner implemented | Infrastructure/support-node installer | Current planner selects the independent-service-unit topology and references the existing isolated service-unit projection without creating/enrolling a node; remaining acceptance requires clean headless-host installation, explicit role selection without automatic network enrollment, per-unit identities, Grid-only durable state, default-deny networking, observability, backup/restore, rotation, quarantine/revocation, update/rollback, decommission, and community-testnet reproduction evidence |
| EDU-001 | Complete for current Mesh/Education synchronization; continuous adoption/retest remains | Keep Axiom Education first-class and independently releasable while synchronized to Mesh | Governed learner memory/write/self-read substrate is merged on AXIOM-MESH `main`; Axiom-Education independently records the merged Mesh provenance while retaining byte-pinned runtime contracts and Gateway semantic-seam binding; its Python/curriculum/Android/Windows/macOS/iOS PR verification passed before merge; future relevant Mesh changes must trigger compatibility/adoption review; no automatic production/provider/curriculum/cross-subject authority claim |
