...
| SOCIAL-002 | In progress; S0 #1648 + S1 #1657 merged; S2 #1658 active; browser-core #1659 draft | Sovereign Social Discovery / Feed as an Axiom One daily-use entry surface | Hard eligibility before ranking; user-owned weighted objectives plus chronological baseline; deterministic ordering; explainable selection provenance; semantic/model scores evidence-only; no ranking-as-authority, visibility override, moderation effect, network fetch, federation, or capability promotion; S1 bounded-decision signal binding then Axiom One Feed/Discovery MVP |
...
| AGENT-005 | In progress — PR #1683 | System One operation-candidate selection v0 | Deterministic eligibility before semantic ranking; exact task/operation/manifest evidence binding; bounded Top-K and visibility-preserving fallback; hard context ceiling; trusted-input proposal verification; zero authority/network/credential/runtime/execution effect |
