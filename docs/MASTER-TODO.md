...
- [ ] **M9.4** Move mTLS certificates from `/certs` directory to secret management (e.g. HashiCorp Vault, AWS Secrets Manager) to prevent credential leakage (owner: security+ops, ETA: TBD). — reopened 2026-03-29 — code/document analysis found private keys still tracked in `certs/`; requires key rotation, repo cleanup, and verified secret-manager-only runtime path.
