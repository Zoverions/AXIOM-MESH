...
| ID | Status | Work | Acceptance evidence |
|---|---|---|---|
| UX-001 | Pending | Define and implement a versioned browser/client contract for the authenticated Gateway API | Compatibility tests, generated or hand-reviewed schemas, explicit error vocabulary, request cancellation, idempotency, and no direct Grid/service access |
| UX-002 | Pending | Build `apps/axiom-one/` as the local personal-node browser/PWA shell outside the trusted kernel | Tested onboarding, node health, Ask, Plan, Approvals, Vault, Receipts, Share, Circles, and advanced inspection routes |
