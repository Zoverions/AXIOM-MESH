...
- [x] **P0** Ascension H1 Refactor `ShadowNode.py` into strict Fossil Node archive semantics (no conversational/active contribution paths). — 2026-04-08 — @agent — replaced active contribution flow with immutable epistemic snapshot ingestion/query APIs and fail-closed active-network guard.
- [x] **P0** Ascension H2 Flesh out `hypervisor/src/agents/ascension_entity.py` with SkillCapsule loading and proof-carrying governance proposal envelope. — 2026-04-08 — @agent — implemented capsule-bound inference/proof envelope generation and bicameral submission path integration.
- [x] **P0** Ascension H3 Update `DigitalLegacy.sol` execution path with `deployAscensionEntity(...)` consent-gated trigger on state transition. — 2026-04-08 — @agent — added universal consent protocol gate + ascension deployment request event hook during terminal transition.
- [x] **P0** Legacy H4 Implement 4-tier sovereign legacy modes (`PURGE`, `FOSSIL`, `SNAPSHOT`, `ASCENSION`) with consent-gated state boundaries and intent timelocks. — 2026-04-08 — @agent — updated DigitalLegacy/Consent/Registry contracts with strict mode boundaries, beneficiary rights, and `IntentUnlocked` events.
- [x] **P0** Legacy H5 Enforce bounded Soul Snapshot runtime (frozen context window, pre-mortem RAG only, prompt-injection rejection, time-lock delivery listener). — 2026-04-08 — @agent — refactored ShadowNode + PrivateVault for bounded retrieval and `IntentUnlocked` event delivery pipeline.
- [x] **P0** Governance H6 Enable Ascension Entity post-biological execution lanes (graph-memory writes, task-market submissions, bicameral voting with proof bundle). — 2026-04-08 — @agent — expanded ascension runtime and ZK proof metadata/verifier checks to reject unauthorized external-context drift.
- [x] **P2** Queue hygiene sweep: confirm there are no remaining unchecked non-deployment items in the canonical queue. — 2026-04-08 — @agent — verified only M6.4 deployment tasks remain unchecked; all other execution items are complete.
- [x] **P1** Documentation integrity audit: verify canonical TODO completion claims, supporting tracker alignment, and unchecked checklist drift across docs. — 2026-04-08 — @agent — added automated audit script/report, synced readiness tracker status to canonical queue, and linked audit in docs index.
...
- [x] **P1** EW.3 Implement embodied-runtime guardrails (geofence/tool/time/force constraints + independent emergency halt path) with audit events. — 2026-04-07 — @agent — added scheduler guardrail gates + emergency halt enforcement + JSONL audit events/tests.
- [x] **P2** EW.4 Create sentience-uncertainty safeguard protocol (detection triggers, escalation board workflow, protected mode) and wire to governance event logs. — 2026-04-07 — @agent — added protocol doc + scheduler enforcement and governance JSONL event logging for sentience escalation/protected mode.
- [x] **P2** EW.5 Add embodied governance evidence gate: release fails without proof of policy enforcement + approval trace for high-risk autonomous actions. — 2026-04-07 — @agent — added high-risk autonomous evidence gate checks in scheduler + CI workflow/script for embodied governance evidence bundle validation.
...
- [x] **P0** M21.1 Sandbox isolation hardening: add gVisor/Kata runtime profile with policy toggle and fail-closed fallback tests. — 2026-04-08 — @agent — sandbox runtime now supports `SANDBOX_RUNTIME_PROFILE` + strict fail-closed/fallback behavior with dedicated docker runner coverage. — 50ba346
- [x] **P0** M21.2 mTLS trust bootstrap/rotation runbook: publish node-join, CA rotation, revocation, and clock-skew response procedure with automation hooks. — 2026-04-08 — @agent — added `docs/HOWTO/mtls-bootstrap-rotation.md` and linked it in HOWTO index with verification hooks for mTLS and evidence checks. — 78a9ccb
- [x] **P0** M21.3 Fail-closed chaos suite: add partition/hypervisor-crash/grid-finality-delay scenarios and gate release on deterministic deny-by-default behavior (owner: ops+reliability, ETA: 2026-04-25). — 2026-04-08 — @agent — added deterministic chaos suite script, CI gate workflow, and release-evidence HOWTO linkage.
- [x] **P1** M21.4 Formal contract verification gate: integrate Slither + Foundry invariant/fuzz checks into CI and expose `make verify-formal` target (owner: contracts+infra, ETA: 2026-04-20). — 2026-04-08 — @agent — added `make verify-formal`, Foundry fuzz/invariant test coverage, and CI formal gate wiring.
...
- [x] **EW.3** Implement embodied action guardrails (geofence/tool/time/force caps + independent emergency halt) with immutable enforcement logs (owner: core+sandbox+security, ETA: TBD). — 2026-04-07 — @agent — implemented guardrail policy checks in task scheduling path with append-only audit log emission and emergency halt fail-closed behavior.
- [x] **EW.4** Publish sentience-uncertainty safeguard protocol and governance escalation workflow (owner: governance+research+safety, ETA: TBD). — 2026-04-07 — @agent — published `docs/governance/SENTIENCE-UNCERTAINTY-SAFEGUARD-PROTOCOL.md` and wired runtime escalation events to governance audit log.
- [x] **EW.5** Add release evidence checks for high-autonomy deployments (approval trace + guardrail verification + incident drill evidence) (owner: ops+security+governance, ETA: TBD). — 2026-04-07 — @agent — added evidence JSON template, validation script, and GitHub Actions gate for high-autonomy governance evidence.
...
- [x] **M9.4** Move mTLS certificates from `/certs` directory to secret management (e.g. HashiCorp Vault, AWS Secrets Manager) to prevent credential leakage (owner: security+ops, ETA: TBD). — 2026-03-29 — @agent — removed tracked cert/private key artifacts and disabled file-path cert fallback across services in favor of env-secret-only mTLS injection.
...
- [x] **M10.9** Founders Council Framework: Create a dual-seat system (human + AI agent) with verifiable recognition via NFTs. Build UI and backend capabilities for recommendations leveraging Soulbound Reputation. — 2026-03-27 — @agent — created FoundersCouncil.sol and gateway dashboard for the council
...
- [x] **M13.6** Complete DASHBOARD-INTERFACES implementation: Wire endpoints to real Grid ledger/Hypervisor telemetry data (owner: gateway+grid). — 2026-03-29 — @agent — wired `/api/v1/dashboard` trust + pipeline payloads to live Grid (`/nodes`, `/skills`, `/zk-stats`, `/dem`) and Hypervisor (`/metrics`) telemetry
- [x] **M13.7** Complete EDUCATION-CAPSULE implementation: Validate and evolve `schemas/education_tome.capnp` and implement production-grade multi-agent persona runtimes (childhood psychologist, guidance counselor, subject expert) (owner: sandbox+ai). — 2026-03-29 — @agent — evolved education_tome schema with persona/maturity context and implemented policy-backed runtime for psychologist/counselor/subject-expert personas
- [x] **M13.8** Complete PAYROLL-INTEGRATION: Add UI/Gateway endpoints for payroll queries and claims, investigate fiat off-ramp/tax compliance integrations (owner: gateway+compliance). — 2026-03-29 — @agent — added gateway payroll API endpoints for pending amounts, claim history, employee registry mutation, and on-chain claim submission
- [x] **M13.9** Complete SUBMISSION-STRUCTURES: Implement credential-gated submission review system and merge workflow into canonical repository/IPFS registry (owner: governance+gateway). — 2026-03-29 — @agent — added Gateway submission registry endpoints for submit/review/merge lifecycle with canonical path + IPFS CID tracking.

- [x] **M12.9** Penetration testing, Load & stress tests, and Cross-chain failure mode drills. (Owner: security+ops). — 2026-03-29 — @agent — published runbook and validated core security/reliability drills with existing verification scripts.
...
- [x] **M17.2** Documentation Auto-generation: Fix critical missing documentation and establish auto-generated versioned docs site from code comments (owner: release, ETA: TBD). — 2026-03-27 — @agent — fixed generate-docs Makefile target.
- [x] **M17.3** Comprehensive Test Coverage: Achieve 80%+ unit test coverage + integration tests across all services (owner: all, ETA: TBD). — 2026-03-27 — @agent — added sandbox execute tests and fixed hypervisor tests, increasing coverage path.
