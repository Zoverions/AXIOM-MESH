...
The same normalized assurance digest is bound into
`intentRequestBinding` / `intentRequestDigest` as `agent_assurance_digest` so
Gateway idempotency and Hypervisor approval fingerprints cannot treat an
assurance-denied request and an assurance-non-deny request as the same effect.
Presence versus absence of assurance evidence also changes request identity.
This remains deny-only identity material; it never grants authority.
