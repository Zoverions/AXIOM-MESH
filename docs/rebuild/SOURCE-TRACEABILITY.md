...
| Node/storage/offline | node-profile, storage-offer, scaling/privacy, degraded-mode, and state-management documents | Capability-aware nodes, security profiles, storage offers, backup/restore, causal sync, conflict visibility | Local signed node/offer lifecycles, encrypted backup/restore, admitted-node discovery/scheduling, offline causal packages, operator-approved two-Grid online causal exchange, and deployment-independent signed secret/policy startup implemented; vendor custody, workload/content transfer, replicated Grid, and federation remain unavailable |
