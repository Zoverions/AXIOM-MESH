...
| Gateway client | client contract/schema/library | route parity/compatibility/error/cancel/timeout/response-bound/real-stack tests | All 32 authenticated Gateway routes are versioned; no direct internal-service target |
...
| Service network policy | network policy/request authorizer/unit Compose | policy and required/forbidden-edge tests | Default deny, 43 exact routes, derived mTLS peers, four internal segments; Education adds one exact Hypervisor-to-Grid POST and no public ingress |
...
| Release/documentation | release verifier/check-docs/current-state doc tests/workflows | `npm run setup`, `npm run release:verify`, protected CI | Canonical docs/links, 32 Gateway routes, 43 network routes, capability counts, runtime-adapter lock, current narrative invariants |
