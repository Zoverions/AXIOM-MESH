...
| Agent runtime adapter candidate | `docs/architecture/AGENT-RUNTIME-ADAPTER-CONFORMANCE.md` and byte-pinned v1 JSON Schema | exact contract verifier, signed-grant negative tests, 28-case synthetic drill, and commit-bound `verify` artifact | No external runtime, live Gateway adapter route, external effect, production trust anchor, or capability promotion |
