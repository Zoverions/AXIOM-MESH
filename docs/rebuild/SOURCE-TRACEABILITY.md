...
| AXIOM One | `apps/axiom-one/`, explanation contract/presenter, proxy, shared Social feed preview adapter/core | policy/static/explanation/approval/uncertainty/feed/real-stack tests | Experimental bounded owner-memory/provenance shell plus owner-local chronological/ranked Social feed preview; session-only objective controls; no remote feed candidates, Social browser mutation, federation, or product promotion |
