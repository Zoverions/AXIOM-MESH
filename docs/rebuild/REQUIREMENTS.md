...
| ID | Requirement | Acceptance evidence |
|---|---|---|
| SECRET-01 | Secret material MUST be governed as a distinct high-sensitivity class. Plaintext passwords, reusable tokens, private/signing keys, recovery material, and comparable secrets MUST NOT enter ordinary model memory, prompts, logs, telemetry, receipts, general export, or browser storage by default. | Storage/context/log/telemetry/export inspection plus malicious-input leakage tests. |
| SECRET-02 | Permission to use a secret MUST NOT imply permission to reveal, copy, export, persist, log, place in model context, or delegate that secret. Reveal, export, and delegation are separately named capabilities and MUST fail closed when not explicitly implemented and authorized. | Secret-use contract tests plus use-to-reveal/export/delegate negative fixtures. |
| SECRET-03 | A secret-use grant MUST bind one authenticated principal, one secret reference, one operation, one purpose, one non-wildcard destination, issue/expiry, policy decision, execution-time authorization recheck, secret-revocation recheck, and receipt obligation. The first promoted profile SHOULD be one-use. | Grant-schema validation, wrong-principal/purpose/destination, expiry, replay, concurrent-use, revocation, and missing-receipt tests. |
| SECRET-04 | Secret custody MAY be AXIOM-native local, external-provider, or hardware-backed, but provider connection, discovery, installation, authentication, custody location, or secret-reference possession MUST NOT mint secret-use or effect authority. | Provider/custody substitution and connection-without-grant denial tests. |
| SECRET-05 | Agents and untrusted runtimes SHOULD receive opaque secret references rather than secret bytes. Brokers SHOULD prefer non-exportable operations, brokered protocol use, or short-lived derived credentials before bounded Sandbox-only plaintext materialization. | Requester-output inspection, model-context tests, process/env/file leakage tests, and provider-specific conformance. |
| SECRET-06 | Secret revocation or rotation MUST block new use immediately and MUST NOT cause an old grant to authorize replacement material. In-flight use MUST be bounded by the applicable cancellation/kill policy. | Rotation/revocation race, replacement-substitution, stale-grant, and cancellation tests. |
| SECRET-07 | General Personal Agent Pack, ordinary portability, receipts, and normal backup manifests MUST remain free of plaintext secret material. Secret recovery or custody migration MUST use a separately reviewed encrypted and destination-bound process. | Export/backup inspection, wrong-recipient/wrong-key, recovery, rollback, and provider-migration tests. |
| SECRET-08 | Secret reveal, export, or authority delegation MUST remain unavailable until each exact path has its own consequence classification, owner-facing confirmation, destination/recipient binding, independent security review, and promotion decision. | Capability-absence tests now; later exact-path review dossier and adversarial suite before promotion. |
...
| AI-07 | Agent orchestration MUST be a separately versioned immutable capsule binding implementation and SBOM digests, interfaces, requested authority, budgets, stop conditions, fallback, cancellation, evidence, uninstall, and rollback. | Capsule schema, conformance, revocation, exhaustion, and rollback tests. |
| AI-08 | A Personal Agent Pack MUST keep encrypted memory, policy, preferences, runtime selection, model adapters, evaluation, licences, and recovery portable without containing plaintext credentials or requiring one base model or managed provider. | Cross-provider export/import, secret inspection, licence, deletion, and recovery tests. |
| AI-09 | Compute routing MUST apply privacy, consent, destination, jurisdiction, licence, security, capability, health, freshness, deadline, and budget as hard filters before ranking eligible choices by usefulness, latency, reliability, cost, or energy. | Forbidden-fallback, stale-candidate, budget, and deterministic-selection tests. |
| AI-10 | Routing and evaluation telemetry MUST exclude raw prompts, audio, credentials, identity attributes, payment data, and memory objects by default; shared learning MUST be explicit, purpose-bound, revocable, and unable to alter authority. | Telemetry inspection, consent, revocation, and policy property tests. |

## Personal devices and Local Trust Plane

| ID | Requirement | Acceptance evidence |
|---|---|---|
| DEVICE-01 | Every wearable or companion endpoint MUST have a unique revocable identity, authenticated pairing, signed firmware/software identity, replay defense, bounded capabilities, expiry, update, rollback, loss, and removal behavior. | Copied-device, wrong-owner, downgrade, replay, stale-session, revocation, update, and rollback tests. |
| DEVICE-02 | An audio or camera endpoint MUST expose an independently testable physical privacy control and an unmistakable active-capture indication that no model or capsule can suppress. | Electrical/firmware inspection, bypass tests, and human comprehension evidence on the exact hardware. |
| DEVICE-03 | A wearable MUST NOT hold provider secrets, raw payment credentials, unrestricted Grid access, or durable personal memory, and the first mobile prototype MUST use an authenticated companion relay rather than direct cellular authority. | Storage, network, pairing, loss, and secret-leakage inspection. |
| DEVICE-04 | Voice or face matching MUST NOT be the sole authorization factor for payment, identity disclosure, public communication, deletion, credential release, or embodied control. | Spoof, replay, accessibility, fallback, and high-risk denial tests. |
| DEVICE-05 | Firmware compilation, simulation, or bench operation MUST NOT be represented as RF, battery, charging, acoustic, electrical, thermal, mechanical, manufacturing, accessibility, usability, or consumer-safety validation. | Claim checks and external evidence tied to exact production hardware. |
| TRUST-01 | Identity, authorization, budget, mandate, replay, and settlement-state decisions MUST use canonical structured inputs, deterministic rules, and named verifiers; a generative model MUST NOT participate in the allow/deny calculation. | Determinism, model-unavailable, prompt-injection, tamper, and verifier-substitution tests. |
| TRUST-02 | Identity proofing, authentication, credential verification, local authorization, external authorization, settlement, reversal, and recovery MUST remain separately named states with no synthetic transition. | State-machine, timeout, late-success, reversal, and claim-parity tests. |
| TRUST-03 | A local mandate MUST be short-lived, one-use, replay-resistant, audience-bound, and signed over exact requester, subject, action, resource, data or transaction, policy, credential, confirmation, budget, nonce, and expiry digests. | Mutation, wrong-audience, expiry, reuse, wrong-key, and partial-binding tests. |
| TRUST-04 | Missing, invalid, revoked, expired, or insufficiently fresh credential, trust-anchor, policy, consent, authenticator, resource, or settlement status MUST fail closed as denied or uncertain according to the exact contract. | Dependency-loss, offline, stale-cache, revocation, and recovery tests. |
| TRUST-05 | Identity presentation MUST use one named credential and presentation profile, holder binding, issuer trust, status/revocation, minimum disclosure, correlation analysis, explicit consent, verifier versioning, and readable non-claims. | Interoperability vectors plus forged issuer, wrong holder, over-disclosure, replay, expiry, revocation, and stale-status tests. |
| TRUST-06 | Reusable payment credentials MUST remain behind a reviewed token or custody boundary and MUST NOT enter the wearable, browser storage, Personal Agent Pack, Grid evidence, logs, prompts, model context, or export. | Storage/network inspection, log and evidence scanning, malicious-input tests, and custody review. |
| TRUST-07 | Payment processing MUST bind integer amount, currency, payee, fee ceiling, purpose, credential handle, adapter, confirmation, budget reserve, idempotency, expiry, cancellation, refund, reversal, dispute, uncertain outcome, reconciliation, and final receipt. | Amount/payee mutation, duplicate, timeout, late-success, forged-receipt, reserve-drift, refund, reversal, and reconciliation tests. |
| TRUST-08 | Real identity or value use MUST remain disabled until the exact issuer, verifier, payment adapter, processor role, custody, privacy, accessibility, support, incident, consumer-protection, legal, regulatory, and jurisdictional boundaries pass qualified independent review. | Domain-specific signed promotion dossier; synthetic or sandbox evidence is insufficient. |
