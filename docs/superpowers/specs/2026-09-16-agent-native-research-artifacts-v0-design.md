...
The offline reproduction binding assessor checks internal consistency among a supplied manifest, operation candidate, and evidence receipt. When the operation declares reference outputs, an attempted receipt must bind its expected outputs to that reference set. A zero-attempt receipt may omit expected outputs, but any supplied nonempty expected-output set must match the declared references, independent of disposition. Empty reference outputs remain unspecified; they do not assert that no expectations are permitted. Its `manifest_claims_current` result reflects only the supplied manifest's own currentness assertion. The records are self-digested, not independently authenticated; the assessor neither runs a method nor checks the claimed observed outputs or tolerance. A receipt's `pass` disposition remains a claim, and stale, corrected, retracted, or withdrawn source evidence is historical only. Unknown currentness stays undetermined.
