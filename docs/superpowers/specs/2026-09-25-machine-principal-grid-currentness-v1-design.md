...
- effective status at `observed_at` (including derived natural expiry);
- sequence;
...
Hypervisor before issuance. This identifies the one intended execution attempt;
Sandbox's `execution_epoch` remains a process-generation binding and must not be
misused as an attempt identifier.
