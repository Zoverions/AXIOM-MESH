...
`revoked`, `compromised`, and `expired` are terminal retained lifecycle states in v1.

Natural `expires_at` remains an independent hard authority ceiling. A retained
`active` or `narrowed` head does not keep a non-persistent machine alive after
its normalized effective authority expiry. Currentness resolution and logical
effect release must compare that expiry to their local observation time and
deny when it has elapsed. The `machine.currentness.expired` transition is an
explicit durable terminal record; it is not required for the time ceiling to
take effect, and reads must not mutate Grid merely to materialize natural time
passage.
...
Initialization rejects a non-persistent root whose `expires_at` is already at
or before the Grid-observed authorization time.
...
Lifecycle storage and replay need a strict **authority snapshot normalizer** that
validates canonical machine-authority shape and digest without requiring an
historical `expires_at` to still be in the future at verification time. Liveness
is a separate check against an explicit observation time. The existing public
principal normalizer keeps its current future-expiry admission behavior.
...
`command_id` is not a free client field. Hypervisor derives it deterministically
from the accepted lifecycle intent id:

```text
machine_cmd_ + sha256(intent_id)
```

The command digest is derived from the exact normalized signed mutation
statement. `command_id` is unique in Grid materialized state.
...
- root authority digest;
- `retained_status`;
- `effective_status` at `observed_at` (including derived natural expiry);
- sequence;
...
- `observed_at` response timestamp;
- explicit non-claims.
...
effective authority digest used for issuance. For a non-persistent effective
authority, capability expiry must be the earlier of the ordinary capability TTL
and the machine authority `expires_at`; Hypervisor must never mint a capability
whose `exp` outlives that authority ceiling.

Each capability also carries a unique `execution_attempt_id` generated by
Hypervisor before issuance and bound into the signed capability. This identifies
the one intended execution attempt. Sandbox's `execution_epoch` remains a
process-generation binding and must not be misused as an attempt identifier.
Logical release binds both values.
...
5. bind exact capability, intent, plan, action, destination,
   `execution_attempt_id`, Sandbox `execution_epoch`, and lifecycle head;
...
A logical release record binds both the unique `execution_attempt_id` and the
Sandbox `execution_epoch`; neither field may be substituted or replayed under a
...
- elapsed root/effective `expires_at` even when retained lifecycle status is still active/narrowed;
- capability expiry later than the effective machine authority expiry;
- reused/mismatched `execution_attempt_id` or Sandbox `execution_epoch`;
...
- expiry may shorten but not extend within the same non-persistent lifetime class;
- any lifetime-class change denies in v1;
- retained active/narrowed state is effectively expired once `expires_at <= observed_at`;
