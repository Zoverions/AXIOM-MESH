...
**Architecture:** S0 adds only science-specific organizational/proposal envelopes. Existing Epistemic `Source`/`Claim`/`Evidence` remain the canonical knowledge/evidence objects, merged Research Capsule v0 (#1603) remains the source/operation/reproduction substrate, and merged Research Composition Graph v0 (#1636) remains the reusable zero-authority lineage/disagreement substrate. The new verifier uses the existing `canonical.mjs` discipline; it performs no provider, network, process, credential, Grid, capability, or filesystem effects beyond reading test fixtures.

**Tech Stack:** Node.js ESM on repository-supported Node versions (`>=22.23.2 <23 || >=24.14.0 <25`), built-in `node:test`, JSON Schema Draft 2020-12 documents, existing `mesh/src/lib/canonical.mjs`, existing Epistemic Fabric E0/E1 contracts, merged Research Capsule v0 (#1603), zero third-party runtime dependencies.
...
- Build from current `main`, where Research Capsule v0 (#1603) and Research Composition Graph v0 (#1636) are already merged; do not fork or duplicate either substrate.
...
**Interfaces:**
- Consumes: existing `epistemic-contracts.mjs` and merged Research Capsule v0 contracts only after the missing-science-module assertion.
