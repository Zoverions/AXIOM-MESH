...
Grid. The current `0.12.0-dev.2` build implements a single-node authority and
...
for four bounded actions: non-consequential echo, owner-scoped private memory
creation, confirmation-bound tombstoning, and selective local memory export.
It provides reversible review, maps every stable Gateway outcome and current
kernel event kind, distinguishes active, expired, consumed, and unknown
...
real-stack negative evidence against cross-principal read, export, and
tombstone. It does not mislabel this projection as an authoritative
pre-execution kernel plan. Hard deletion, restore, bulk ingestion, general
...
The `0.12.0-dev.2` build does not claim:
