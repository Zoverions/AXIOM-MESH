...
| Owner-local social substrate | In progress: intent-authorized local actor/persona/publication create/supersede/retract plus owner-derived `/v1/social`; A2 publication projection; AXIOM One S2A chronological/ranked owner-local feed preview; no federation or network distribution | Complete S2A exact-head UI/accessibility evidence, then separately gate stored remote candidate eligibility; later exchange/federation requires a separate protocol/security gate |
...
| AXIOM One browser/PWA shell | Experimental (`UX-002`): loopback-only shell, contract-only proxy, memory-only token, governed bounded Ask/Vault/receipt views, owner-local Feed / Discovery preview with session-only objectives and chronological fallback | Complete remote candidate eligibility separately, onboarding, session/device security, broader browser/accessibility/usability fixtures, signed package/update/rollback/uninstall/support |
