...
| Remote social review inspection | Candidate mechanism implemented; exact-head CI pending: owner-only `/v1/social/remote-review`, owner derived from authenticated principal, query overrides rejected, minimized G5A projection, accepted `SocialGridStore`, no remote-schema creation and no social/network/authority effect | Require exact-head Clean Kernel/Windows/chain evidence before PR readiness; any staging/admission/follow/transport/federation remains a separate gate |
