# HOWTO Runbook Index

This directory contains executable operational guides. Steps should be copy/paste runnable unless otherwise stated.

## Core Operations
- `run-local-stack.md` — bring up local services and verify health.
- `submit-intent.md` — submit intent via gateway and trace response path.
- `contracts-local.md` — compile/test/deploy contracts locally.
- `release-gate-evidence.md` — assemble and validate RC evidence package.

## Existing Functional HOWTOs
- `swarm-join.md`
- `zkml-infer.md`
- `recovery-2fa.md`
- `nemoclaw-policy.md`
- `founder-claim.md`
- `meshstore-claim.md`

## Coverage policy
If a feature requires operator action, it must have a HOWTO in this directory.
