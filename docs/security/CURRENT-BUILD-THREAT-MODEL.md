...
| Machine principal expires while a request is in flight | Gateway admission checks declared expiry before handler execution, and constrained application responses re-check the same declared expiry before application bytes are written; direct constrained response methods are blocked | The recheck uses local wall-clock and the authenticated principal record loaded for the request. Live revocation/currentness and atomic authorization/disclosure ordering remain separate requirements; suppressing a response does not undo handler effects |
