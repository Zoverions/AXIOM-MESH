...
| Social | Reads the authenticated owner's bounded local actor/persona/publication corpus and presents an active Feed / Discovery preview with chronological mode, transparent ranked mode, session-only objective priorities, exact "Why am I seeing this?" contributions, append-only history, and raw source projections | `social.get`; shared browser-safe S0 ranking core | Read-only owner-local projection; chronological remains available; invalid ranked evidence fails closed to chronological; ranking changes display order only; remote-review objects are not candidates; no federation, remote distribution, browser preference persistence, or browser Social mutation |
