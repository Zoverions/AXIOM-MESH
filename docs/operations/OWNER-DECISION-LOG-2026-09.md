...
The Epistemic Fabric Stage 5B work is a separate design-gated track. The owner approved the original E0/E1 gate, but a pre-implementation standards review found that its exact composed JSON Schema bytes would reject specialized fields under standard JSON Schema 2020-12 semantics. Implementation is therefore blocked pending explicit approval of narrow schema-composition amendment A. No E0/E1 code has been authorized against the amended bytes yet, and no authority for E2+ phases exists. Stage 5A artifacts remain inputs/provenance only.
