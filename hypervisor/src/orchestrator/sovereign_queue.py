...
class SovereignExecutionQueue:
    ...
    def __init__(self, rpc_endpoint: str, contract_address: str):
        ...
        # Signing authority is always provisioned explicitly. A missing key
        # disables reporting instead of simulating a successful chain write.
    ...
    async def report_execution_state(self, proposal_id: str, success: bool, artifact_hash: str):
        ...
        if not self.account:
            raise RuntimeError("EXECUTOR_PRIVATE_KEY is required to report an on-chain execution")
        try:
            def _send_tx():
