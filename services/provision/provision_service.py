...
try:
    ...
    print("Missing dependency: qrcode. Install declared dependencies before startup.")
    sys.exit(1)
...
def sign_payload(payload: Dict[str, Any], secret: Optional[str] = None) -> str:
    ...
    secret = provisioning_secret(secret)
...
def provisioning_secret(explicit: Optional[str] = None) -> str:
    """Return an explicitly provisioned, non-example HMAC secret."""
    secret = explicit or os.environ.get("AXIOM_MESH_SECRET")
    if not secret or len(secret) < 32:
        raise RuntimeError("AXIOM_MESH_SECRET must be set to at least 32 characters")
    return secret
